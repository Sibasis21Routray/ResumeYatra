...
export function EditorPanel({ sidebarOpen }: { sidebarOpen: boolean }) {
  ...
  // Scroll to top when section changes (next/previous navigation)
useEffect(() => {
  ...
  const handleCustomSectionNavigate = (sectionId: string) => {
    if (validateSectionForNavigation(sectionId)) {
      ...
      toast.error("Please complete the current section before jumping ahead.");
    }
  };
  ...
  return (
    <>
      ...
      <div
        className={`flex-1 bg-white dark:bg-gray-800 ${!isMobile ? "" : "w-full"
          }`}
      >
        <div className="flex-1 overflow-y-auto  dark:bg-gray-900">
          <div className="w-full pl-6 sm:py-8 lg:py-10">
