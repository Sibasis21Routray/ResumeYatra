...
import { Edit, Trash2, Mic, Building2, Calendar, FileText, AlertCircle, Plus, ChevronRight } from "lucide-react";
...
interface SpeakingEngagementItem {
    id: string;
    topic: string;
    eventName: string;
    date: string;
...
interface SpeakingEngagementsFormProps {
    onBack?: () => void;
    onNext?: () => void;
...
// Speaking Engagement Card Component for Summary View
const SpeakingEngagementCard = ({ engagement, onEdit, onDelete }: { 
    engagement: SpeakingEngagementItem; 
    onEdit: () => void; 
    onDelete: () => void;
}) => (
        ...
        <div className="flex justify-between items-start">
            <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                    <Mic className="w-5 h-5 text-accent dark:text-dark-accent" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                    {engagement.eventName && (
                        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-dark-text-muted">
                            ...
                            <span>{engagement.eventName}</span>
                        </div>
                    )}
                    {engagement.date && (
                        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-dark-text-muted">
                ...
                {engagement.description && (
                    <div className="mt-3 p-3 bg-bg-secondary dark:bg-dark-bg-secondary rounded-lg">
                        <p className="text-sm text-text-primary dark:text-dark-text-primary">
                            {engagement.description.length > 100 
                                ? engagement.description.substring(0, 100) + '...' 
                                : engagement.description}
                        </p>
                    </div>
                )}
...
export function SpeakingEngagementsForm({
    ...
    const [tempItem, setTempItem] = useState<Partial<SpeakingEngagementItem>>({});
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [touched, setTouched] = useState<Record<string, boolean>>({});

    const engagements = data.speakingEngagements || [];

    const isSummaryView = engagements.length > 0 && !isEditing;
    ...
    useEffect(() => {
        if (engagements.length === 0) {
            setIsEditing(true);
            setTempItem({ id: `speaking-${Date.now()}` });
        }
    }, [engagements.length]);
    ...
    const validateForm = () => {
        ...
        if (!tempItem.topic?.trim()) {
            newErrors.topic = "Topic/Session Title is required";
        } else if (tempItem.topic.length > 120) {
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("Are you sure you want to delete this speaking engagement?")) {
            updateData((draft) => {
                if (draft.speakingEngagements) {
                    draft.speakingEngagements = draft.speakingEngagements.filter((a) => a.id !== id);
                }
            });
            toast.success('Speaking engagement deleted successfully!', {
                style: toastStyle.success,
                duration: 2000,
            });
    ...
    const updateField = (field: string, value: any) => {
        ...
        if (field === "topic" && errors.topic) {
            setErrors((prev:any) => ({ ...prev, topic: undefined }));
        }
    };
    ...
    const handleBlur = (field: string) => {
        ...
        // Validate on blur
        if (field === "topic" && !tempItem.topic?.trim()) {
            setErrors((prev) => ({ ...prev, topic: "Topic/Session Title is required" }));
        }
    };
        ...
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    Speaking Engagements <span className="text-accent dark:text-dark-accent">Summary</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Review and manage your speaking engagements and presentations.
                </p>
            </div>
            ...
            <div className="mt-6">
                ...
                >
                    <Plus className="w-5 h-5" /> Add more speaking engagements
                </button>
            </div>
    ...
    const renderForm = () => (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Showcase your public speaking experience and presentations.
                </p>
            </div>
