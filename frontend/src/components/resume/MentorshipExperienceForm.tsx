...
import { Edit, Trash2, AlertCircle, Plus, Users, Building, UserCheck, Calendar } from "lucide-react";
...
interface MentorshipExperienceItem {
    id: string;
    mentorshipArea: string; // Mentorship Area (120 chars) - required - Career, Technical, Academic, etc.
    organizationPlatform: string; // Organization/Platform (120 chars) - optional - Institute, NGO, platform
    menteeLevel: string; // Mentee Level (80 chars) - optional - Students, Professionals
    duration: string; // Duration (50 chars) - optional - Mentorship period
    description: string; // Description (200 chars) - optional - Mentorship summary
}

interface MentorshipExperienceFormProps {
    onBack?: () => void;
    onNext?: () => void;
...
// Mentorship Experience Card Component for Summary View
const MentorshipExperienceCard = ({ experience, onEdit, onDelete }: { 
    experience: MentorshipExperienceItem; 
    onEdit: () => void; 
    onDelete: () => void;
}) => (
        ...
        <div className="flex justify-between items-start">
            <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                    <Users className="w-5 h-5 text-accent dark:text-dark-accent" />
                    <h3 className="font-semibold text-lg text-text-primary dark:text-dark-text-primary">
                        {experience.mentorshipArea}
                    </h3>
                </div>
                
                <div className="grid grid-cols-1 gap-2 mb-2">
                    {experience.organizationPlatform && (
                        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-dark-text-muted">
                            <Building className="w-4 h-4" />
                            <span>{experience.organizationPlatform}</span>
                        </div>
                    )}
                    {experience.menteeLevel && (
                        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-dark-text-muted">
                            <UserCheck className="w-4 h-4" />
                            <span>{experience.menteeLevel}</span>
                        </div>
                    )}
                    {experience.duration && (
                        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-dark-text-muted">
...
export function MentorshipExperienceForm({
    onBack,
    onNext,
    onNavigateToSection,
}: MentorshipExperienceFormProps) {
    ...
    const [tempItem, setTempItem] = useState<Partial<MentorshipExperienceItem>>({});
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [touched, setTouched] = useState<Record<string, boolean>>({});

    const experiences = data.mentorshipExperience || [];
    ...
    useEffect(() => {
        if (experiences.length === 0) {
            setIsEditing(true);
            setTempItem({ id: `mentorship-${Date.now()}` });
        }
    }, [experiences.length]);
    ...
    const validateForm = () => {
        ...
        if (!tempItem.mentorshipArea?.trim()) {
            newErrors.mentorshipArea = "Mentorship Area is required";
        } else if (tempItem.mentorshipArea.length > 120) {
            newErrors.mentorshipArea = "Mentorship Area must be less than 120 characters";
        }

        if (tempItem.organizationPlatform && tempItem.organizationPlatform.length > 120) {
            newErrors.organizationPlatform = "Organization/Platform must be less than 120 characters";
        }

        if (tempItem.menteeLevel && tempItem.menteeLevel.length > 80) {
            newErrors.menteeLevel = "Mentee Level must be less than 80 characters";
        }
    ...
    const isFormValid = () => {
        return tempItem.mentorshipArea?.trim() !== "";
    };
    ...
    const handleSubmit = () => {
        ...
        updateData((draft) => {
            if (!draft.mentorshipExperience) draft.mentorshipExperience = [];
            if (editingId) {
                const index = draft.mentorshipExperience.findIndex((e) => e.id === editingId);
                if (index !== -1) {
                    draft.mentorshipExperience[index] = tempItem as MentorshipExperienceItem;
                }
                toast.success('Mentorship experience updated successfully!', {
                    style: toastStyle.success,
                    duration: 2000,
                });
            } else {
                draft.mentorshipExperience.push(tempItem as MentorshipExperienceItem);
                toast.success('Mentorship experience added successfully!', {
                    style: toastStyle.success,
                    duration: 2000,
                });
            }
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("Are you sure you want to delete this mentorship experience?")) {
            updateData((draft) => {
                if (draft.mentorshipExperience) {
                    draft.mentorshipExperience = draft.mentorshipExperience.filter((e) => e.id !== id);
                }
            });
            toast.success('Mentorship experience deleted successfully!', {
                style: toastStyle.success,
                duration: 2000,
            });
    ...
    const updateField = (field: string, value: any) => {
        ...
        if (field === "mentorshipArea" && errors.mentorshipArea) {
            setErrors((prev) => ({ ...prev, mentorshipArea: undefined }));
        }
    };
    ...
    const handleBlur = (field: string) => {
        ...
        // Validate on blur
        if (field === "mentorshipArea" && !tempItem.mentorshipArea?.trim()) {
            setErrors((prev) => ({ ...prev, mentorshipArea: "Mentorship Area is required" }));
        }
    };
        ...
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    Mentorship Experience <span className="text-accent dark:text-dark-accent">Summary</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Review and manage your mentorship experience.
                </p>
            </div>
            ...
            <div className="mt-6">
                ...
                >
                    <Plus className="w-5 h-5" /> Add more mentorship experience
                </button>
            </div>
    ...
    const renderForm = () => (
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    {editingId ? "Edit" : "Add"} <span className="text-accent dark:text-dark-accent">Mentorship Experience</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Add details about your mentorship experience.
                </p>
            </div>
            ...
            <div className="bg-bg-primary dark:bg-dark-bg-primary rounded-xl border border-light-border dark:border-dark-border p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <StyledInput
                        ...
                        error={touched.mentorshipArea ? errors.mentorshipArea : ""}
                    />
                    ...
                    <StyledInput
                        ...
                        error={touched.organizationPlatform ? errors.organizationPlatform : ""}
                    />
                    ...
                    <StyledInput
                        ...
                        error={touched.menteeLevel ? errors.menteeLevel : ""}
                    />
                    ...
                    <div className="md:col-span-2">
                        <StyledTextArea
                            ...
                            placeholder="Mentorship summary..."
                            maxLength={200}
                            rows={3}
...
export default MentorshipExperienceForm;