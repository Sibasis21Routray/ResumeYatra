...
interface CareerEnhancerFormProps {
    onBack?: () => void;
    onNext?: () => void;
    onNavigateToSection?: (section: string) => void;
}

export function CareerEnhancerForm({
    onBack,
    onNext,
    onNavigateToSection,
}: CareerEnhancerFormProps) {
    const navigate = useNavigate();
    const { id } = useParams();

    const careerEnhancerOptions = [
        ...
        <div className="mb-8">
            <div className="relative bg-gradient-to-r from-[#04477E] to-[#0a5a9e] rounded-xl p-6 mb-6 shadow-lg">
                ...
                <div className="relative flex flex-col sm:flex-row sm:justify-between sm:items-center">
                    <div>
                        <h3 className="text-xl font-bold text-white mb-1">Career Enhancer</h3>
                        <div className="text-white/80 text-sm">Add professional credentials.</div>
                    </div>
                    <div className="mt-3 sm:mt-0">
                        <div className="bg-white/20 backdrop-blur-sm rounded-lg px-3 py-1.5">
                            <span className="text-white text-sm font-medium">5 Options</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {careerEnhancerOptions.map((option) => (
                    <button
                        key={option.name}
                        onClick={() => {
