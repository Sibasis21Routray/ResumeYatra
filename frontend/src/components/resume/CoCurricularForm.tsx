...
import { Edit, Trash2, Trophy, Star, Calendar, AlertCircle, Plus, Award } from "lucide-react";
...
export function CoCurricularForm({
    onBack,
    onNext,
}: CoCurricularFormProps) {
    ...
    const [tempActivity, setTempActivity] = useState<Partial<CoCurricularItem>>({});
    const [errors, setErrors] = useState<{ activity?: string }>({});
    const [isYearDropdownOpen, setIsYearDropdownOpen] = useState(false);

    const activities = data.coCurricular || [];
    ...
    useEffect(() => {
        if (activities.length === 0) {
            setIsEditing(true);
            setTempActivity({ id: `cocurricular-${Date.now()}` });
        }
    }, [activities.length]);
    ...
    const handleSubmit = () => {
        ...
        updateData((draft) => {
            if (!draft.coCurricular) draft.coCurricular = [];
            if (editingId) {
                const index = draft.coCurricular.findIndex((a) => a.id === editingId);
                if (index !== -1) {
                    draft.coCurricular[index] = tempActivity as CoCurricularItem;
                }
                ...
                draft.coCurricular.push(tempActivity as CoCurricularItem);
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("Are you sure you want to delete this activity?")) {
            updateData((draft) => {
                if (draft.coCurricular) {
                    draft.coCurricular = draft.coCurricular.filter((a) => a.id !== id);
                }
            });
        ...
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    Co-curricular <span className="text-accent dark:text-dark-accent">Summary</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Review and manage your co-curricular activities.
                </p>
            </div>
            ...
            <div className="mt-6">
                ...
                >
                    <Plus className="w-5 h-5" /> Add more co-curricular activities
                </button>
            </div>
        </div>
    );
    ...
    const renderForm = () => (
        <div className="space-y-6">
            ...
            <SectionCard 
                ...
                description="Information about your co-curricular activity"
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <StyledInput
                        label="Activity Name"
                        placeholder="e.g., Debate Club, College Football Team, Music Society"
                        ...
                        helperText="Debate Club, College Football Team, Music Society"
                    />

                    <StyledInput
                        label="Role / Achievement"
                        placeholder="e.g., Captain, Winner, Lead"
                        value={currentActivity.role || ""}
                        onChange={(e) => updateField("role", e.target.value)}
                        maxLength={120}
                        characterCount
                        icon={<Award className="w-4 h-4" />}
                        helperText="Role held or recognition received (if any)"
                    />
                </div>
            </SectionCard>
