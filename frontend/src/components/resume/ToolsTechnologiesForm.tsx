...
interface Tool {
    id: string;
    name: string; // Tool/Technology Name (120 chars) - Name of tool or technology
    category: string; // Category (80 chars) - Programming, Analytics, CRM, etc.
    proficiency: string; // Proficiency Level (Dropdown: Beginner, Intermediate, Advanced, Expert)
    experienceDuration: string; // Experience Duration (50 chars) - e.g., 3 years
}

interface ToolsTechnologiesFormProps {
    onBack?: () => void;
    onNext?: () => void;
...
// Tool Card Component for Summary View
        ...
        <div className="flex justify-between items-start">
            <div className="flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                    ...
                    {tool.proficiency && (
                        <div className="text-sm text-text-muted dark:text-dark-text-muted">
                            <span className="font-medium">Proficiency:</span> {tool.proficiency}
                        </div>
                    )}
                    {tool.experienceDuration && (
                        <div className="text-sm text-text-muted dark:text-dark-text-muted">
                            <span className="font-medium">Experience:</span> {tool.experienceDuration}
                        </div>
                    )}
                </div>
            </div>
...
export function ToolsTechnologiesForm({
    onBack,
    onNext,
    onNavigateToSection,
}: ToolsTechnologiesFormProps) {
    ...
    const [tempItem, setTempItem] = useState<Partial<Tool>>({});
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [touched, setTouched] = useState<Record<string, boolean>>({});

    const tools = data.toolsTechnologies || [];

    const isSummaryView = tools.length > 0 && !isEditing;

    // Proficiency levels for dropdown
    ...
    const validateForm = () => {
        ...
        if (!tempItem.name?.trim()) {
            newErrors.name = "Tool/Technology Name is required";
        } else if (tempItem.name.length > 120) {
            newErrors.name = "Tool/Technology Name must be less than 120 characters";
        }

        if (!tempItem.category?.trim()) {
            newErrors.category = "Category is required";
        } else if (tempItem.category.length > 80) {
            newErrors.category = "Category must be less than 80 characters";
        }

        if (!tempItem.proficiency) {
    ...
    const isFormValid = () => {
        return tempItem.name?.trim() !== "" && 
               tempItem.category?.trim() !== "" && 
               tempItem.proficiency?.trim() !== "";
    };
    ...
    const handleSubmit = () => {
        ...
        updateData((draft) => {
            if (!draft.toolsTechnologies) draft.toolsTechnologies = [];
            if (editingId) {
                const index = draft.toolsTechnologies.findIndex((t) => t.id === editingId);
                if (index !== -1) {
                    draft.toolsTechnologies[index] = tempItem as Tool;
                }
                toast.success('Tool/Technology updated successfully!', {
                    style: toastStyle.success,
                    duration: 2000,
                });
            } else {
                draft.toolsTechnologies.push(tempItem as Tool);
                toast.success('Tool/Technology added successfully!', {
                    style: toastStyle.success,
                    duration: 2000,
                });
            }
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("Are you sure you want to delete this tool/technology?")) {
            updateData((draft) => {
                if (draft.toolsTechnologies) {
                    draft.toolsTechnologies = draft.toolsTechnologies.filter((t) => t.id !== id);
                }
            });
            toast.success('Tool/Technology deleted successfully!', {
                style: toastStyle.success,
                duration: 2000,
            });
    ...
    const updateField = (field: string, value: any) => {
        ...
        if (field === "proficiency" && errors.proficiency) {
    ...
    const handleBlur = (field: string) => {
        ...
        if (field === "name" && !tempItem.name?.trim()) {
            setErrors((prev) => ({ ...prev, name: "Tool/Technology Name is required" }));
        }
        
        if (field === "category" && !tempItem.category?.trim()) {
            setErrors((prev) => ({ ...prev, category: "Category is required" }));
        }
        
        if (field === "proficiency" && !tempItem.proficiency) {
            setErrors((prev) => ({ ...prev, proficiency: "Proficiency Level is required" }));
        }
    };
        ...
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    Tools & Technologies <span className="text-accent dark:text-dark-accent">Summary</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Review and manage your tools and technologies.
                </p>
            </div>
            ...
            <div className="mt-6">
                ...
                >
                    <Plus className="w-5 h-5" /> Add more tools and technologies
                </button>
            </div>
    ...
    const renderForm = () => (
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-dark-text-primary mb-2">
                    {editingId ? "Edit" : "Add"} <span className="text-accent dark:text-dark-accent">Tool or Technology</span>
                </h2>
                <p className="text-base text-text-muted dark:text-dark-text-muted">
                    Add tools and technologies you're proficient in.
                </p>
            </div>
            ...
            <div className="bg-bg-primary dark:bg-dark-bg-primary rounded-xl border border-light-border dark:border-dark-border p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <StyledInput
                        ...
                        tooltip="Name of tool or technology"
                    />

                    <StyledInput
                        label="Category"
                        placeholder="e.g., Programming, Analytics, CRM"
                        ...
                        tooltip="Programming, Analytics, CRM, etc."
                    />
