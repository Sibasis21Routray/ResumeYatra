...
interface PersonalUtilityFormProps {
    onBack?: () => void;
    onNext?: () => void;
    onNavigateToSection?: (section: string) => void;
}

export function PersonalUtilityForm({
    onBack,
    onNext,
    onNavigateToSection,
}: PersonalUtilityFormProps) {
    const navigate = useNavigate();
    const { id } = useParams();

    const personalUtilityOptions = [
        ...
        <div className="mb-8">
            <div className="relative bg-gradient-to-r from-[#04477E] to-[#0a5a9e] rounded-xl p-6 mb-6 shadow-lg">
                ...
                <div className="relative flex flex-col sm:flex-row sm:justify-between sm:items-center">
                    <div>
                        <h3 className="text-xl font-bold text-white mb-1">Personal / Utility Sections</h3>
                        <div className="text-white/80 text-sm">Add personal and utility information.</div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {personalUtilityOptions.map((option) => (
                    <button
                        key={option.name}
                        onClick={() => {
