...
const ensureIds = (data: any) => {
  const arrayKeys = [
    "experience", "projects", "education", "internships", "trainingPrograms",
  ...
  for (const key of arrayKeys) {
    if (Array.isArray(processed[key])) {
      processed[key] = processed[key].map((item: any, index: number) => {
        if (typeof item === 'object' && item !== null) {
          if (!item.id && !item._id) {
            item.id = `${key}-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 9)}`;
          ...
          if (key === 'customSections' && Array.isArray(item.entries)) {
            item.entries = item.entries.map((entry: any, eIndex: number) => {
              if (typeof entry === 'object' && entry !== null && !entry.id && !entry._id) {
                entry.id = `entry-${Date.now()}-${eIndex}-${Math.random().toString(36).substring(2, 9)}`;
              } else if (typeof entry === 'object' && entry !== null && entry._id && !entry.id) {
