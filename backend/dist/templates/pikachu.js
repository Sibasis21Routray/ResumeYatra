...
<div class="container">
  ...
  <div class="content-wrapper">
      ...
      <div class="section" data-section="languages">
        ...
        <div class="horizontal-line"></div>
        <ul>
          ${data.languages.map((lang) => `<li>${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      <div class="section" data-section="hobbies">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 15px;">
          ${data.hobbies.map((hobby) => `<div style="background: #f1f5f9; color: #475569; padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 500;">${hobby}</div>`).join('')}
        </div>
      </div>` : ''}
      ...
      <div class="section" data-section="socialLinks">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-top: 15px;">
          ${data.socialLinks.map((link) => `<a href="${link.url}" target="_blank" style="color: var(--primary-color); text-decoration: none; font-weight: 500; font-size: ${baseFontSize}px;">${link.urlText || link.url.replace('https://', '').replace('http://', '')}</a>`).join('')}
        </div>
      </div>` : ''}
      ...
      <div class="section" data-section="keyAchievements">
        ...
        <div class="horizontal-line"></div>
        <ul>
          ${data.keyAchievements.map((achievement) => `<li>${achievement}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      `;
        ...
        <div class="section" data-section="customSections">
          ...
          <div class="entry-content">
            <ul>
              ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, entryIndex) => `<li data-section="customSections" data-index="${entryIndex}">${entry.title || entry.organization || entry.description || 'Achievement'}</li>`).join('') : '<li style="color: #64748b; font-style: italic;">No achievements added</li>'}
            </ul>
          </div>
        </div>
      `;
        ...
        <div class="section" data-section="customSections">
          ...
          <div class="entry-content">
            <ul>
              ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, entryIndex) => `<li data-section="customSections" data-index="${entryIndex}">${entry.title || entry.organization || entry.description || 'Responsibility'}</li>`).join('') : '<li style="color: #64748b; font-style: italic;">No responsibilities added</li>'}
            </ul>
          </div>
        </div>
      `;
        ...
        <div class="section" data-section="customSections">
          ...
          <div class="entry-content">
            <ul>
              ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, entryIndex) => `<li data-section="customSections" data-index="${entryIndex}">${entry.title || entry.organization || entry.description || 'Tool'}</li>`).join('') : '<li style="color: #64748b; font-style: italic;">No tools added</li>'}
            </ul>
          </div>
        </div>
      `;
