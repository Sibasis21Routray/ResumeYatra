...
exports.buildSeniorIndividualContributorTemplate = buildSeniorIndividualContributorTemplate;
function buildSeniorIndividualContributorTemplate(data, theme) {
    const defaultTheme = {
        primary: "#2C3E50",
        secondary: "#34495E",
        background: "#ffffff",
    ...
    const headerTextColor = headerBg.toLowerCase() === "#ffffff" ? "#000000" : "white";
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<div class="page">
  ...
  <div class="header" data-section="personal">
    <div class="header-content">
      ...
      <div class="tagline">${data.careerObjective ||
        data.summary ||
        "Professional with diverse experience"}</div>
      ${data.professionalContext?.totalExperience
        ...
        : data.experience && data.experience.length > 0
            ? `<div class="experience-label">Total Experience: ${calculateTotalExperience(data.experience)}</div>`
            : ""}
    </div>
    ${data.personal?.image
        ? `<img src="${data.personal.image}" alt="${data.personal?.name && data.personal?.name !== "undefined"
            ? data.personal.name
            : "Profile Photo"}" style="width: 140px; height: 140px; object-fit: cover; border-radius: 4px; margin-left: 20px; flex-shrink: 0;">`
  ...
  <div class="content-wrapper">
    <div class="main-content">
      ...
      ${sortedExperience.length > 0
        ...
        ${sortedExperience
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${exp.startDate || ""} - ${exp.endDate || (exp.isCurrent ? "Present" : "")}</div>
            ${exp.description
      ...
      ${data.trainingPrograms && data.trainingPrograms.length > 0
        ...
        ${data.trainingPrograms
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.provider || ""}${item.duration ? ` | ${item.duration}` : ""}</div>
      ...
      ${data.academicProjects && data.academicProjects.length > 0
        ...
        ${data.academicProjects
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.name || ""}</div>
            <div class="company-info">${item.course || ""}${item.institution ? ` | ${item.institution}` : ""}</div>
            <div class="company-info">${item.duration || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
            ${item.technologies && item.technologies.length > 0
            ? `<div style="margin-top: 5px;"><strong>Technologies:</strong> ${item.technologies.join(", ")}</div>`
            : ""}
            ${item.url
      ...
      ${data.leadershipPositions && data.leadershipPositions.length > 0
        ...
        ${data.leadershipPositions
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.startDate || ""} - ${item.endDate || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
      ...
      ${data.clientProjects && data.clientProjects.length > 0
        ...
        ${data.clientProjects
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.name || ""}</div>
            <div class="company-info">${item.clientOrganization || ""}${item.role ? ` | Role: ${item.role}` : ""}</div>
            <div class="company-info">${item.duration || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
      ...
      ${data.portfolio && data.portfolio.length > 0
        ...
        ${data.portfolio
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.name || ""}</div>
            <div class="company-info">${item.type || ""}${item.platform ? ` | ${item.platform}` : ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
            ${item.url
      ...
      ${data.volunteering && data.volunteering.length > 0
        ...
        ${data.volunteering
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.organization || ""}${item.causeArea ? ` | ${item.causeArea}` : ""}</div>
            <div class="company-info">${item.duration || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
      ...
      ${data.militaryService && data.militaryService.length > 0
        ...
        ${data.militaryService
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.duration || ""}${item.specialization ? ` | ${item.specialization}` : ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
      ...
      ${data.teachingExperience && data.teachingExperience.length > 0
        ...
        ${data.teachingExperience
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.title || ""}</div>
            <div class="company-info">${item.institution || ""}${item.subjectCourseTaught ? ` | ${item.subjectCourseTaught}` : ""}</div>
            <div class="company-info">${item.duration || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
      ...
      ${data.mentorshipExperience && data.mentorshipExperience.length > 0
        ...
        ${data.mentorshipExperience
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.organizationPlatform || ""}${item.menteeLevel ? ` | ${item.menteeLevel}` : ""}</div>
            <div class="company-info">${item.duration || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
      ...
      ${data.researchGrants && data.researchGrants.length > 0
        ...
        ${data.researchGrants
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.title || ""}</div>
            <div class="company-info">${item.agency || ""}${item.amount ? ` | Amount: ${item.amount}` : ""}</div>
      ...
      ${data.publications && data.publications.length > 0
        ...
        ${data.publications
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.title || ""}</div>
            <div class="company-info">${item.journalPublisher || ""}${item.publicationType ? ` | ${item.publicationType}` : ""}</div>
            <div class="company-info">${item.year || ""}${item.authors ? ` | Authors: ${item.authors}` : ""}</div>
            ${item.urlDoi
      ...
      ${data.patents && data.patents.length > 0
        ...
        ${data.patents
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.title || ""}</div>
            <div class="company-info">${item.patentNumber || ""}${item.issuingAuthority ? ` | ${item.issuingAuthority}` : ""}</div>
            <div class="company-info">${item.year || ""} | Status: ${item.status || ""}</div>
          </div>
        `)
            .join("")}
      </div>`
      ...
      ${data.awards && data.awards.length > 0
        ...
        ${data.awards
          ...
          <div class="experience-item" data-index="${index}">
            <div class="job-title">${item.title || ""}</div>
            <div class="company-info">${item.organization || ""} | ${item.issueYear || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
      ...
      ${data.speakingEngagements && data.speakingEngagements.length > 0
        ...
        ${data.speakingEngagements
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.eventName || ""} | ${item.date || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
      ...
      ${data.memberships && data.memberships.length > 0
        ...
        ${data.memberships
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.organizationName || ""} | ${item.year || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
      ...
      ${data.workshops && data.workshops.length > 0
        ...
        ${data.workshops
          ...
          <div class="experience-item" data-index="${index}">
            ...
            <div class="company-info">${item.conductedBy || ""} | ${item.year || ""}</div>
            ${item.description
            ? `<div class="section-content">${item.description}</div>`
            : ""}
          </div>
    ...
    <div class="sidebar">
        ...
        <ul class="skills-list">
          ${(() => {
            ...
            if (typeof skills === 'string' && skills.includes('<ul>')) {
                ...
                return skills.split(',').map((skill) => `<li>${skill.trim()}</li>`).join('');
            }
      ...
      ${data.toolsTechnologies && data.toolsTechnologies.length > 0
        ...
        <ul class="skills-list">
          ${data.toolsTechnologies
            .map((item) => `<li>${item.name || ""}${item.proficiency ? ` (${item.proficiency})` : ""}${item.experienceDuration ? ` - ${item.experienceDuration}` : ""}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
      ...
      ${data.methodologies && data.methodologies.length > 0
        ...
        <ul class="skills-list">
          ${data.methodologies
            .map((item) => `<li>${item.name || ""}${item.certification ? ` (${item.certification})` : ""}${item.experienceDuration ? ` - ${item.experienceDuration} years` : ""}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
      ...
      ${data.industryExpertise && data.industryExpertise.length > 0
        ...
        <ul class="skills-list">
          ${data.industryExpertise
            .map((item) => `<li>${item.industry || ""}${item.domainArea ? ` - ${item.domainArea}` : ""}${item.experienceDuration ? ` (${item.experienceDuration} years)` : ""}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
      ...
      ${data.languages && data.languages.length > 0
        ...
        <ul class="skills-list">
          ${data.languages
            .map((lang) => `<li>${lang.language || ""}${lang.level ? ` (${lang.level})` : ""}${lang.capability ? ` - ${lang.capability}` : ""}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
      ...
      ${data.socialProfiles && data.socialProfiles.length > 0
        ...
        ${data.socialProfiles
            .map((profile) => `<div class="contact-item"><a href="${profile.url}" target="_blank">${profile.platform || "Profile"}</a></div>`)
            .join("")}
      </div>`
        : ""}
