...
exports.buildStartupAndTechTemplate = buildStartupAndTechTemplate;
function buildStartupAndTechTemplate(data, theme) {
    ...
    const formatDateRangeInline = (startDate, endDate, isCurrent) => {
        const parts = [];
        if (startDate?.trim())
    ...
    const diamond = (size = 16, color = diamondColor) => `<svg width="${size}" height="${size}" viewBox="0 0 16 16" style="display:inline-block;vertical-align:middle;flex-shrink:0;"><rect x="2" y="2" width="12" height="12" transform="rotate(45 8 8)" fill="${color}"/></svg>`;
    const smallDiamond = (size = 10, color = diamondColor) => `<svg width="${size}" height="${size}" viewBox="0 0 10 10" style="display:inline-block;vertical-align:middle;flex-shrink:0;"><rect x="1" y="1" width="8" height="8" transform="rotate(45 5 5)" fill="${color}"/></svg>`;
    const skillArray = typeof skills === "string"
        ? skills.split(",").map(s => s.trim()).filter(s => s)
        : Array.isArray(skills) ? skills : [];
    const linkedIn = socialProfiles.find((p) => p.platform?.toLowerCase().includes("linkedin"))?.url || "";
    const github = socialProfiles.find((p) => p.platform?.toLowerCase().includes("github"))?.url || "";
    // Filter arrays
    ...
    const sectionHeader = (title) => `<div class="section-header" style="position:relative; margin-left:0;">
    ...
    const timelineItem = (dateStr, titleHtml, subtitleHtml, bodyHtml, index) => `<div class="timeline-item" ${index !== undefined ? `data-index="${index}"` : ''}>
    ...
    <body>
      ...
      <div class="resume-body">
        ...
        return timelineItem(dateStr, `<div class="timeline-title">${exp.title || ''}</div>`, `<div class="timeline-subtitle">${exp.company || ''}${exp.location ? ` | ${exp.location}` : ''}</div>`, exp.description ? renderBullets(exp.description) : '', idx);
    }).join('')}
        </div>
        ` : ''}
        ...
        return timelineItem(dateStr, `<div class="timeline-title">${item.title || ''}</div>`, `<div class="timeline-subtitle">${item.company || ''}</div>`, item.description ? renderBullets(item.description) : '', idx);
    }).join('')}
        </div>
        ` : ''}
        ...
        return timelineItem(dateStr, `<div class="edu-degree">${edu.degree || edu.course || ''}${edu.field ? `: ${edu.field}` : ''}</div>`, `<div class="edu-school">${edu.school || edu.university || ''}${edu.location ? ` | ${edu.location}` : ''}</div>`, `${edu.grade ? `<div class="edu-grade">Grade: ${edu.grade}</div>` : ''}${edu.description ? renderBullets(edu.description) : ''}`, idx);
    }).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="certifications">
          ${nonEmptyCertifications.map((cert, idx) => timelineItem(cert.year || '', `<div class="timeline-title">${cert.name || ''}</div>`, cert.issuer ? `<div class="timeline-subtitle">${cert.issuer}</div>` : '', '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="projects">
          ${nonEmptyProjects.map((project, idx) => timelineItem(project.duration || '', `<div class="timeline-title">${project.name || project.title || ''}</div>`, project.role ? `<div class="timeline-subtitle">${project.role}</div>` : '', (project.description ? renderBullets(project.description) : '') +
        (project.technologies ? `<p class="info-text"><strong>Technologies:</strong> ${project.technologies}</p>` : ''), idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="academicProjects">
          ${nonEmptyAcademicProjects.map((item, idx) => timelineItem(item.duration || '', `<div class="timeline-title">${item.name || item.title || ''}</div>`, item.institution ? `<div class="timeline-subtitle">${item.institution}</div>` : '', item.description ? renderBullets(item.description) : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="trainingPrograms">
          ${nonEmptyTrainingPrograms.map((item, idx) => timelineItem(item.completionDate || '', `<div class="timeline-title">${item.name || ''}</div>`, item.provider || item.organization ? `<div class="timeline-subtitle">${item.provider || item.organization}</div>` : '', '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="leadershipPositions">
          ${nonEmptyLeadershipPositions.map((item, idx) => timelineItem(formatDateRangeInline(item.startDate, item.endDate), `<div class="timeline-title">${item.position || item.title || ''}</div>`, `<div class="timeline-subtitle">${item.organization || ''}</div>`, item.description ? renderBullets(item.description) : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <ul class="simple-list" data-section="activities">
          ${[...nonEmptyCoCurricular, ...nonEmptyExtracurricular].map((item, idx) => `
            <li data-index="${idx}">
              ${smallDiamond(9, diamondColor)}
        ...
        <div class="timeline-wrapper" data-section="awards">
          ${nonEmptyAwards.map((item, idx) => timelineItem(item.year || '', `<div class="timeline-title">${item.title || ''}</div>`, item.organization ? `<div class="timeline-subtitle">${item.organization}</div>` : '', item.description ? `<p class="info-text">${item.description}</p>` : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="publications">
          ${nonEmptyPublications.map((item, idx) => timelineItem(item.year || '', `<div class="timeline-title">${item.title || ''}</div>`, item.journalPublisher ? `<div class="timeline-subtitle">${item.journalPublisher}</div>` : '', item.authors ? `<p class="info-text">${item.authors}</p>` : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="volunteering">
          ${nonEmptyVolunteering.map((item, idx) => timelineItem(formatDateRangeInline(item.startDate, item.endDate), `<div class="timeline-title">${item.role || ''}</div>`, `<div class="timeline-subtitle">${item.organization || ''}</div>`, item.description ? renderBullets(item.description) : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="speakingEngagements">
          ${nonEmptySpeakingEngagements.map((item, idx) => timelineItem(item.date || item.year || '', `<div class="timeline-title">${item.title || item.topic || ''}</div>`, `<div class="timeline-subtitle">${item.event || item.organization || ''}</div>`, '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="workshops">
          ${nonEmptyWorkshops.map((item, idx) => timelineItem(item.date || '', `<div class="timeline-title">${item.name || item.title || ''}</div>`, `<div class="timeline-subtitle">${item.organizer || item.organization || ''}</div>`, '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="teachingExperience">
          ${nonEmptyTeachingExperience.map((item, idx) => timelineItem(formatDateRangeInline(item.startDate, item.endDate), `<div class="timeline-title">${item.subjectCourseTaught || item.title || ''}</div>`, `<div class="timeline-subtitle">${item.institution || ''}</div>`, '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="researchGrants">
          ${nonEmptyResearchGrants.map((item, idx) => timelineItem(item.amount || '', `<div class="timeline-title">${item.title || ''}</div>`, `<div class="timeline-subtitle">${item.agency || ''}</div>`, '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="militaryService">
          ${nonEmptyMilitaryService.map((item, idx) => timelineItem(formatDateRangeInline(item.startDate, item.endDate), `<div class="timeline-title">${item.rank || ''}${item.role ? ` — ${item.role}` : ''}</div>`, `<div class="timeline-subtitle">${item.branch || ''}</div>`, '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="clientProjects">
          ${nonEmptyClientProjects.map((item, idx) => timelineItem(item.duration || '', `<div class="timeline-title">${item.clientName || item.name || ''}</div>`, item.projectName ? `<div class="timeline-subtitle">${item.projectName}</div>` : '', item.description ? renderBullets(item.description) : '', idx)).join('')}
        </div>
        ` : ''}
        ...
        <div class="timeline-wrapper" data-section="references">
          ${nonEmptyReferences.map((item, idx) => timelineItem('', `<div class="timeline-title">${item.name || ''}</div>`, `<div class="timeline-subtitle">${item.designationRelationship || ''}${item.organization ? ` at ${item.organization}` : ''}</div>`, [item.email, item.phone].filter(Boolean).map(v => `<p class="info-text">${v}</p>`).join(''), idx)).join('')}
        </div>
        ` : ''}
