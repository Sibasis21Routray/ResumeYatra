...
function buildMetricsDrivenTemplate(data, theme) {
    ...
    const toLines = (v) => {
        if (!v)
            return [];
        if (Array.isArray(v))
            return v.map(String).filter(l => l.trim());
        return String(v).split('\n').filter(l => l.trim());
    };
    const show = (section) => data.sectionVisibility?.[section] !== false;
    // ── Derived ───────────────────────────────────────────────────────────────
    ...
    const expAchievements = (item) => {
        if (!item.achievements)
            return [];
        if (Array.isArray(item.achievements))
            return item.achievements.filter((a) => String(a).trim());
    ...
    const renderSkillsSidebar = () => {
        ...
        <div class="skill-item" data-section="skills" data-index="${i}">• ${typeof skill === 'object'
    ...
    const contactFooter = () => {
        const items = [];
        if (hasValue(data.personal?.phone))
            items.push(`<span data-section="personal" data-index="0">📞 ${s(data.personal.phone)}</span>`);
        if (hasValue(data.personal?.alternatePhone))
            items.push(`<span data-section="personal" data-index="1">📞 ${s(data.personal.alternatePhone)}</span>`);
        if (hasValue(data.personal?.email))
            items.push(`<span data-section="personal" data-index="2">✉️ ${s(data.personal.email)}</span>`);
        ...
        if (loc)
            items.push(`<span data-section="personal" data-index="3">📍 ${loc}</span>`);
        if (hasValue(data.personal?.linkedinUrl))
            items.push(`<span data-section="personal" data-index="6">🔗 <a href="${s(data.personal.linkedinUrl)}" target="_blank">LinkedIn</a></span>`);
        if (hasValue(data.personal?.githubUrl))
            items.push(`<span data-section="personal" data-index="7">💻 <a href="${s(data.personal.githubUrl)}" target="_blank">GitHub</a></span>`);
        if (hasValue(data.personal?.portfolioUrl))
            items.push(`<span data-section="personal" data-index="8">🌐 <a href="${s(data.personal.portfolioUrl)}" target="_blank">Portfolio</a></span>`);
        if (hasValue(data.personal?.website))
            items.push(`<span data-section="personal" data-index="9">🌐 <a href="${s(data.personal.website)}" target="_blank">Website</a></span>`);
        if (hasValue(data.personal?.twitter))
            items.push(`<span>🐦 <a href="${s(data.personal.twitter)}" target="_blank">Twitter</a></span>`);
        if (hasValue(data.personal?.skype))
            items.push(`<span>💬 Skype: ${s(data.personal.skype)}</span>`);
        return items.join('');
    };
    return `<!DOCTYPE html>
<html lang="en">
  ...
  <div class="main-grid">
    ...
    <div class="sidebar">
      ...
      <div class="sidebar-section" data-section="keyAchievements">
        ...
        ${data.keyAchievements.map((a, i) => `<div class="skill-item" data-section="keyAchievements" data-index="${i}">• ${s(a)}</div>`).join('')}
      </div>` : ''}
      ...
      <div class="sidebar-section" data-section="languages">
        ...
        <div style="font-size:9pt;color:rgba(22,21,21,0.9);">
          ${data.languages.map((lang) => typeof lang === 'string' ? lang : `${s(lang.language)}${hasValue(lang.level) ? ` (${s(lang.level)})` : ''}`).join(', ')}
        </div>
      </div>` : ''}
      ...
      <div class="sidebar-section" data-section="socialProfiles">
        ...
        <div style="font-size:9pt;color:rgba(46,44,44,0.9);display:flex;flex-direction:column;gap:5px;">
          ${data.socialProfiles.map((item, i) => `
      ...
      <div class="sidebar-section" data-section="industryExpertise">
        ...
        ${data.industryExpertise.map((item, i) => `
      ...
      <div class="sidebar-section" data-section="toolsTechnologies">
        ...
        ${data.toolsTechnologies.map((item, i) => `
      ...
      <div class="sidebar-section" data-section="responsibilities">
        ...
        <ul style="color:rgba(31,30,30,0.9);">
          ${responsibilityLines.map((l, i) => `<li style="font-size:9pt;" data-section="responsibilities" data-index="${i}">${l.trim()}</li>`).join('')}
        </ul>
      </div>` : ''}
    ...
    <div class="content">
      ...
      <div class="main-section" data-section="experience">
        ...
        return `
          <div class="exp-item" data-section="experience" data-index="${index}">
            ...
            ${lines.length > 0 ? `<ul>${lines.map((l, li) => `<li data-section="experience" data-index="${index}" data-item-index="${li}">${l.trim()}</li>`).join('')}</ul>` : ''}
            ${achievements.length > 0 ? `<ul style="margin-top:4px;">${achievements.map((a, ai) => `<li data-section="experience" data-index="${index}" data-item-index="${ai}">⭐ ${a}</li>`).join('')}</ul>` : ''}
      ...
      <div class="main-section" data-section="education">
        ...
        ${data.education.map((edu, index) => `
          <div class="education-entry" data-section="education" data-index="${index}">
            ...
            ${(Array.isArray(edu.achievements) && edu.achievements.filter((a) => a.trim()).length > 0) ? `
              <div class="education-achievements" data-section="education" data-index="${index}">
                <h4>Academic Excellence</h4>
                <ul>${edu.achievements.filter((a) => a.trim()).map((a, ai) => `<li data-section="education" data-index="${index}" data-item-index="${ai}">${a}</li>`).join('')}</ul>
              </div>` : ''}
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="internships">
        ...
        return `
          <div class="exp-item" data-section="internships" data-index="${index}">
            ...
            ${lines.length > 0 ? `<ul>${lines.map((l, li) => `<li data-section="internships" data-index="${index}" data-item-index="${li}">${l.trim()}</li>`).join('')}</ul>` : ''}
      ...
      <div class="main-section" data-section="speakingEngagements">
        ...
        ${data.speakingEngagements.map((item, index) => `
      ...
      <div class="main-section" data-section="clientProjects">
        ...
        ${data.clientProjects.map((item, index) => `
      ...
      <div class="main-section" data-section="militaryService">
        ...
        ${data.militaryService.map((item, index) => `
      ...
      <div class="main-section" data-section="mentorshipExperience">
        ...
        ${data.mentorshipExperience.map((item, index) => `
      ...
      <div class="main-section" data-section="researchGrants">
        ...
        ${data.researchGrants.map((item, index) => `
      ...
      <div class="main-section" data-section="testScores">
        ...
        <div class="badge-list">
          ${data.testScores.map((item, index) => `
