...
exports.buildAzurillTemplate = buildAzurillTemplate;
function buildAzurillTemplate(data, theme) {
    const defaultTheme = {
        primary: '#1e40af',
        secondary: '#ffffff',
        background: '#ffffff',
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      ${(() => {
        ...
        if (data.personal?.fathersName)
            personalDetails.push(`👨 Father: ${data.personal.fathersName}`);
        if (data.personal?.dob)
            personalDetails.push(`📅 DOB: ${data.personal.dob}`);
        if (data.personal?.gender)
            personalDetails.push(`⚥ Gender: ${data.personal.gender}`);
        if (data.personal?.maritalStatus)
            personalDetails.push(`💍 Marital: ${data.personal.maritalStatus}`);
    ...
    <div class="sidebar-section">
      ...
      <ul data-section="skills">
        ${nonEmptySkills.map((s, index) => `<li data-section="skills" data-index="${index}">${typeof s === 'string' ? s : s}</li>`).join('')}
      </ul>
    </div>` : ''}
    ...
    <div class="sidebar-section">
      ...
      ${getNonEmptyArray(data.education).map((edu, index) => `
        <div class="education-item" data-section="education" data-index="${index}">
          ...
          ${edu.graduationDate ? `<div class="education-date" data-section="education" data-index="${index}">${edu.startDate || edu.graduationDate
    ...
    <div class="sidebar-section">
      ...
      <ul data-section="hobbies">
        ${nonEmptyHobbies.map((hobby, index) => `<li data-section="hobbies" data-index="${index}">${hobby}</li>`).join('')}
      </ul>
    </div>` : ''}
