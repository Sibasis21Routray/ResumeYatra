...
exports.buildMinimalTemplate = buildMinimalTemplate;
exports.buildModernTemplate = buildModernTemplate;
function buildMinimalTemplate(data, theme) {
    const defaultTheme = {
        primary: "#2563eb",
        secondary: "#64748b",
        background: "#ffffff",
  ...
  <div class="container">
    ...
    <div class="header" data-section="personal">
      ...
      <div class="contact">
        ...
        ? `<span data-section="personal" data-index="3">${data.personal.location}${data.personal?.country ? ", " + data.personal.country : ""}${data.personal?.pinCode ? ", " + data.personal.pinCode : ""}${data.personal?.fullAddress
    ...
    ${data.internships && data.internships.length > 0
      ...
      ${(data.internships || [])
        ...
        <div class="entry" data-index="${index}">
          ...
          <div class="entry-subtitle">${item.company || ""}${item.location ? `, ${item.location}` : ""}</div>
      ...
      ${(data.projects || [])
        ...
        <div class="entry" data-index="${index}">
          <div class="entry-header">
            ...
            ? `<div class="entry-date">${project.startDate || ""} ${project.startDate && project.endDate
                ? `- ${project.endDate}`
                : project.endDate
                    ? `- ${project.endDate}`
                    : ""}</div>`
    ...
    ${data.sectionVisibility?.socialLinks !== false &&
      ...
      <div style="display: flex; flex-wrap: wrap; gap: 15px;">
        ${data.socialLinks
            .map((link, index) => `
          <a href="${link.url}" target="_blank" data-section="socialLinks" data-index="${index}">${link.urlText ||
    ...
    ${data.trainingPrograms && data.trainingPrograms.length > 0
      ...
      ${(data.trainingPrograms || [])
        ...
        <div class="entry" data-index="${index}">
          ...
          <div class="entry-subtitle">${item.provider || item.organization || ""}${item.duration ? ` • ${item.duration}` : ""}</div>
          <div class="entry-content">${item.description || ""}</div>
        </div>
      `)
            .join("")}
    ...
    ${data.scholarships && data.scholarships.length > 0
      ...
      ${(data.scholarships || [])
        ...
        <div class="entry" data-index="${index}">
          ...
          <div class="entry-subtitle">${item.provider || item.organization || ""}${item.amount ? ` • ${item.amount}` : ""}</div>
          <div class="entry-content">${item.description || ""}</div>
        </div>
      `)
            .join("")}
      ...
      ${(data.coCurricular || [])
        ...
        <div class="entry" data-index="${index}">
          <div class="entry-header">
            ...
            <div class="entry-date">${item.year ||
            (item.startDate ? `${item.startDate} - ${item.endDate || ""}` : "")}</div>
          </div>
          <div class="entry-subtitle">${item.organization || ""}${item.role ? ` • ${item.role}` : ""}</div>
          <div class="entry-content">${item.description || ""}</div>
        </div>
      `)
            .join("")}
