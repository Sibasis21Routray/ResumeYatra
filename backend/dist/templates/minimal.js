...
function buildMinimalTemplate(data, theme) {
    ...
    const renderDescription = (description) => {
      ...
      <div class="custom-list-container" style="margin-top: 5px;">
        ${lines.map(line => `
          <div class="custom-list-item" style="display: table; width: 100%; margin-bottom: 6px; font-size: ${baseFontSize}px; line-height: 1.4; color: var(--text-color);">
...
<body>
  <div class="container">
    ...
    ${data.sectionVisibility?.experience !== false &&
      ...
      ${(data.experience || [])
        ...
        <div class="entry" data-index="${index}">
          ...
          <div class="entry-content">${exp.description ? renderDescription(exp.description) : ""}</div>
    ...
    ${data.sectionVisibility?.education !== false &&
      ...
      ${(data.education || [])
            .map((edu, index) => {
            const dateStr = edu.startDate || edu.graduationDate || edu.endDate
                ? `${edu.startDate || ''}${edu.startDate && (edu.graduationDate || edu.endDate) ? ' - ' : ''}${edu.graduationDate || edu.endDate || (edu.isCurrent ? 'Present' : '')}`
                : '';
            return `
        <div class="entry" data-index="${index}">
          <div class="entry-header">
          ...
          ${edu.school
                ...
                : ""}
          ${edu.grade
                ? `<div class="education-field">Grade/CGPA: ${edu.grade}</div>`
                : ""}
          
          ${edu.description
                ? `
            <div class="education-description" style="margin-top: 5px;">
