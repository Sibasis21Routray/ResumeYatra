...
function buildSkillsFirstTemplate(data, theme) {
    ...
    const hasResponsibilitiesContent = (Array.isArray(data.responsibilities)
        ? data.responsibilities
        : (data.responsibilities || "").split("\n")).filter((line) => line.trim()).length > 0;
    const hasToolsContent = data.tools &&
  ...
  <div class="main-grid">
    <div class="sidebar">
      ...
      <div class="sidebar-box" data-section="socialLinks">
        ...
        ${data.socialLinks
            .map((link, index) => `
          <div class="contact-item" data-section="socialLinks" data-index="${index}">🔗 <a href="${link.url}" target="_blank">${link.urlText ||
...
${data.sectionVisibility?.projects !== false &&
        ...
        ${data.projects
          ...
          <div style="margin-bottom: 15px;" data-section="projects" data-index="${index}">
            ...
            ${project.url
            ? `<p style="font-size: 8pt;"><a href="${project.url}" target="_blank">${project.urlText || "View Project"}</a></p>`
            : ""}
          </div>
        `)
            .join("")}
      ...
      ${data.internships && data.internships.length > 0
        ...
        ${data.internships
          ...
          <div class="exp-item" data-section="internships" data-index="${index}">
            ...
            <div class="role-title">${item.company || ""}${item.location ? `, ${item.location}` : ""}</div>
      ...
      ${data.academicProjects && data.academicProjects.length > 0
        ...
        ${data.academicProjects
          ...
          <div class="exp-item" data-section="academicProjects" data-index="${index}">
            ...
            <div class="role-title">${item.institution || ""}${item.course ? ` | ${item.course}` : ""}</div>
            <p>${item.description || ""}</p>
            ${item.technologies
            ? `<p style="font-size: 9pt;"><strong>Technologies:</strong> ${Array.isArray(item.technologies)
      ...
      ${data.trainingPrograms && data.trainingPrograms.length > 0
        ...
        ${data.trainingPrograms
          ...
          <div class="exp-item" data-section="trainingPrograms" data-index="${index}">
            ...
            <div class="role-title">${item.provider || item.organization || ""}${item.duration ? ` | ${item.duration}` : ""}</div>
            <p>${item.description || ""}</p>
          </div>
        `)
            .join("")}
      ...
      ${data.scholarships && data.scholarships.length > 0
        ...
        ${data.scholarships
          ...
          <div class="exp-item" data-section="scholarships" data-index="${index}">
            ...
            <div class="role-title">${item.provider || item.organization || ""}${item.amount ? ` | ${item.amount}` : ""}</div>
            <p>${item.description || ""}</p>
          </div>
        `)
            .join("")}
      ...
      ${data.coCurricular && data.coCurricular.length > 0
        ...
        ${data.coCurricular
          ...
          <div class="exp-item" data-section="coCurricular" data-index="${index}">
            ...
            <div class="role-title">${item.organization || ""}${item.role ? ` | ${item.role}` : ""}</div>
            <p>${item.description || ""}</p>
          </div>
        `)
            .join("")}
      ...
      ${data.sectionVisibility?.languages !== false &&
        ...
        <p style="font-size: 9pt;">${data.languages
            .map((lang) => `${lang.language || lang}${lang.level ? ` (${lang.level})` : ""}`)
            .join(", ")}</p>
      </div>`
        : ""}
      ...
      ${data.keyAchievements && data.keyAchievements.length > 0
        ...
        <ul>
          ${data.keyAchievements
            .map((achievement, index) => `<li style="font-size: 9pt;" data-section="keyAchievements" data-index="${index}">${achievement}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
      ...
      ${hasResponsibilitiesContent
        ...
        <ul>
          ${(Array.isArray(data.responsibilities)
            ...
            .map((line, index) => `<li style="font-size: 9pt;" data-section="responsibilities" data-index="${index}">${line.trim()}</li>`)
            .join("")}
        </ul>
      </div>`
        : ""}
