...
function buildFresherEntryLevelTemplate(data, theme) {
    ...
    const renderSkillsGrid = (skills) => {
        ...
        if (typeof skills === 'string') {
            ...
            if (skills.includes('<'))
                return `<div style="font-size:${bodyFontSize};">${skills}</div>`;
            // comma-separated plain string
            return skills.split(',').filter((s) => s.trim()).map((s) => `<div class="skill-item">${s.trim()}</div>`).join('');
        }
        if (Array.isArray(skills)) {
            return skills.map((skill) => `<div class="skill-item">${typeof skill === 'object'
                ? s(skill.name) + (skill.level ? ` (${s(skill.level)})` : '')
                : s(skill).trim()}</div>`).join('');
        }
        return '';
    };
    const sectionDiv = (content, label, sectionKey) => `<div class="section" data-section="${sectionKey}">
...
<div class="resume-wrapper">
  ...
  <div class="section" data-section="experience">
    ...
    ${sortedExperience.map((exp, index) => {
      ...
      <div class="experience-item" data-index="${index}">
        ...
        ${achievements.length > 0 ? `<ul>${achievements.map((a, ai) => `<li data-section="experience" data-index="${index}" data-item-index="${ai}"> ${a}</li>`).join('')}</ul>` : ''}
    ...
    ${data.education.map((edu, index) => `
      <div class="education-item" data-index="${index}">
        ...
        ${(Array.isArray(edu.achievements) && edu.achievements.filter((a) => a.trim()).length > 0) ? `
          <ul>${edu.achievements.filter((a) => a.trim()).map((a) => `<li>${a}</li>`).join('')}</ul>` : ''}
      </div>
    `).join('')}
  </div>` : ''}
  ...
  <div class="section" data-section="projects">
    ...
    ${data.projects.map((project, index) => {
      ...
      <div class="experience-item" data-index="${index}">
        ...
        ${lines.length > 0 ? `<ul>${lines.map((l) => `<li>${l.trim()}</li>`).join('')}</ul>` : ''}
  ...
  <div class="section" data-section="methodologies">
    ...
    <hr class="section-divider">
    <div class="badge-row">
      ${data.methodologies.map((item, index) => `
  ...
  <div class="section" data-section="industryExpertise">
    ...
    <hr class="section-divider">
    <div class="badge-row">
      ${data.industryExpertise.map((item, index) => `
  ...
  <div class="section" data-section="tools">
    ...
    <hr class="section-divider">
    <div class="skills-grid">
      ${(Array.isArray(data.tools) ? data.tools : s(data.tools).split(',')).filter((t) => String(t).trim()).map((t) => `<div class="skill-item">${s(t).trim()}</div>`).join('')}
    </div>
  </div>` : ''}
  ...
  <!-- ═══ KEY ACHIEVEMENTS ═══ -->
  ${(show('keyAchievements') && Array.isArray(data.keyAchievements) && data.keyAchievements.filter((a) => a && a.trim()).length > 0) ? `
  <div class="section" data-section="keyAchievements">
    <div class="section-title">Key Achievements</div>
    <hr class="section-divider">
    <ul>${data.keyAchievements.filter((a) => a && a.trim()).map((a, i) => `<li data-section="keyAchievements" data-index="${i}">${a}</li>`).join('')}</ul>
  </div>` : ''}
  ...
  <div class="section" data-section="responsibilities">
    ...
    <hr class="section-divider">
    <ul>${toLines(data.responsibilities).map((l, i) => `<li data-section="responsibilities" data-index="${i}">${l.trim()}</li>`).join('')}</ul>
  </div>` : ''}
  ...
  <div class="section" data-section="languages">
    ...
    <hr class="section-divider">
    <div style="font-size:${bodyFontSize};">
      ${data.languages.map((lang) => `<div class="skill-item">${typeof lang === 'string' ? lang : s(lang.language)}${hasValue(typeof lang === 'object' ? lang.level : '') ? ` (${s(lang.level)})` : ''}${hasValue(typeof lang === 'object' ? lang.capability : '') ? ` — ${s(lang.capability)}` : ''}</div>`).join('')}
    </div>
  </div>` : ''}
  ...
  <div class="section" data-section="hobbies">
    ...
    <hr class="section-divider">
    <div style="font-size:${bodyFontSize};">
      ${data.hobbies.map((h) => `<div class="skill-item">${s(h).trim()}</div>`).join('')}
    </div>
  </div>` : ''}
  ...
  ${(Array.isArray(data.customSections) && data.customSections.length > 0)
        ? data.customSections
            ...
            section.entries.some((e) => e.isVisible !== false))
            .map((section) => `
