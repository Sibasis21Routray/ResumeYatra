...
function buildClassicProfessionalTemplate(data, theme) {
    ...
    const expAchievements = (exp) => {
        if (!exp.achievements)
            return [];
        if (Array.isArray(exp.achievements))
            return exp.achievements.filter((a) => String(a).trim());
...
<div class="resume-container">
  ...
  <div class="main-body">
        ...
        : `<p>${edu.description}</p>`}
            ...
            ${(Array.isArray(edu.achievements) && edu.achievements.filter((a) => a.trim()).length > 0) ? `
              <div class="education-achievements" data-section="education" data-index="${index}">
                ...
                <ul>
                  ${edu.achievements.filter((a) => a.trim()).map((a, ai) => `<li data-section="education" data-index="${index}" data-item-index="${ai}">${a}</li>`).join('')}
                </ul>
              </div>` : ''}
          </div>
        `).join('')}
    ...
    <div class="content">
      ...
      <!-- Career Objective -->
      ${(show('careerObjective') && hasValue(data.careerObjective) && !hasValue(data.summary)) ? `
      ...
      <div class="main-section" data-section="experience">
        ...
        return `
          <div class="exp-item" data-section="experience" data-index="${index}">
            ...
            ${restLines.length > 0 ? `
            <ul>
              ${restLines.map((line, li) => `<li data-section="experience" data-index="${index}" data-item-index="${li}">${line.trim()}</li>`).join('')}
            </ul>` : ''}
          ${expAchievements(exp).length > 0 ? `
            <ul>
              ${expAchievements(exp).map((a, ai) => `<li data-section="experience" data-index="${index}" data-item-index="${ai}">⭐ ${a}</li>`).join('')}
            </ul>` : ''}
      ...
      <div class="main-section" data-section="languages">
        ...
        <p style="font-size: 9pt;">
          ${data.languages.map((lang) => typeof lang === 'string'
        ? lang
        : `${s(lang.language)}${hasValue(lang.level) ? ` (${s(lang.level)})` : ''}`).join(', ')}
        </p>
      </div>` : ''}
      ...
      <div class="main-section" data-section="keyAchievements">
        ...
        <ul>
          ${data.keyAchievements.map((item, index) => `<li style="font-size: 9pt;" data-section="keyAchievements" data-index="${index}">${s(item)}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      <div class="main-section" data-section="responsibilities">
        ...
        <ul>
          ${responsibilityLines.map((line, index) => `<li style="font-size: 9pt;" data-section="responsibilities" data-index="${index}">${line.trim()}</li>`).join('')}
        </ul>
      </div>` : ''}
