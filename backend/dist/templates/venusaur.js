...
exports.buildVenusaurTemplate = buildVenusaurTemplate;
function buildVenusaurTemplate(data, theme) {
    const defaultTheme = {
        primary: "#000000",
        secondary: "#444444",
        background: "#ffffff",
      ...
      ${(() => {
        if (data.personal?.personalInfoDisplay === "inline") {
            ...
            if (data.personal?.fathersName)
                inlineItems.push(`<span data-section="personal">👨 Father: ${data.personal.fathersName}</span>`);
            if (data.personal?.dob)
                inlineItems.push(`<span data-section="personal">📅 DOB: ${data.personal.dob}</span>`);
            if (data.personal?.gender)
                inlineItems.push(`<span data-section="personal">⚥ ${data.personal.gender}</span>`);
            if (data.personal?.maritalStatus)
                inlineItems.push(`<span data-section="personal">💍 ${data.personal.maritalStatus}</span>`);
            if (data.personal?.nationality)
                inlineItems.push(`<span data-section="personal">🌍 ${data.personal.nationality}</span>`);
