...
router.get('/token-usage', async (req, res) => {
    try {
        const usageLogs = await TokenUsage_1.default.find()
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [userCount, resumeCount, activeSubsCount, freelancerCount, candidateCount, revenueResult, aiUsageResult] = await Promise.all([
            User_1.default.countDocuments(),
            Resume_1.default.countDocuments(),
            User_1.default.countDocuments({ subscriptionPlan: { $ne: 'none' }, subscriptionExpiry: { $gt: now } }),
            User_1.default.countDocuments({ subscriptionPlan: 'freelancer', subscriptionExpiry: { $gt: now } }),
            User_1.default.countDocuments({ subscriptionPlan: 'candidate', subscriptionExpiry: { $gt: now } }),
            Invoice_1.default.aggregate([
                { $match: { status: 'paid', createdAt: { $gt: thirtyDaysAgo } } },
                { $group: { _id: null, total: { $sum: '$amount' } } }
            ]),
            TokenUsage_1.default.aggregate([
        ...
        res.json({
            ...
            totalAiEnhancements: await TokenUsage_1.default.countDocuments({ action: { $in: ['enhanceResume', 'enhanceSkillsWithAI'] } }),
            aiAdoptionRate: userCount > 0 ? Math.round((await TokenUsage_1.default.distinct('userId')).length / userCount * 100) : 0
        });
    }
    catch (error) {
...
// Update pricing
router.put('/pricing', pricing_controller_1.updatePricing);
// Upload admin signature
router.post('/settings/signature', upload_middleware_1.uploadMiddleware, pricing_controller_1.updateAdminSignature);
// Delete user
    ...
    try {
        ...
        const formattedResume = {
            ...
            owner: resume.ownerId ? {
                ...
                name: resume.ownerId.name,
                email: resume.ownerId.email
            } : null,
            versions: resume.versions ? resume.versions.map((version) => ({
