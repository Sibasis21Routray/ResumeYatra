...
exports.toggleAutoPay = toggleAutoPay;
exports.handleWebhook = handleWebhook;
exports.getAutoPayStatus = getAutoPayStatus;
exports.getInvoiceByResume = getInvoiceByResume;
...
async function createOrder(req, res) {
    try {
            ...
            if (userId) {
                ...
                if (type === "download") {
                    ...
                    if (user?.subscriptionPlan === "freelancer")
                        discount = pricing.freelancerAiDiscount;
                    else if (user?.subscriptionPlan === "candidate")
                        discount = pricing.candidateAiDiscount;
        ...
        if (useSubscription) {
            const planId = type === "subscription_freelancer" ? pricing.freelancerPlanId : pricing.candidatePlanId;
            if (!planId) {
                ...
                const subscription = await exports.razorpay.subscriptions.create({
                    plan_id: planId,
...
async function verifyPayment(req, res) {
    try {
        const { razorpay_order_id, razorpay_subscription_id, razorpay_payment_id, razorpay_signature, resumeId, type, includeItem, } = req.body;
        const userId = req.userId || null;
        ...
        if (type.startsWith("subscription")) {
            ...
            try {
                await (0, invoice_service_1.createAndSaveInvoice)({
                    userId: user._id,
        ...
        try {
            ...
            if (userId) {
                ...
                const latestVersion = await ResumeVersion_1.default.findOne({ resumeId: resume._id }).sort({ createdAt: -1 });
...
// ==================== AUTO-PAY TOGGLE ====================
async function toggleAutoPay(req, res) {
    try {
        ...
        if (autoPay === false && user.razorpaySubscriptionId) {
            try {
                ...
                await exports.razorpay.subscriptions.cancel(user.razorpaySubscriptionId);
...
async function handleWebhook(req, res) {
    ...
    if (event === "subscription.charged") {
        ...
        const user = await User_1.default.findOne({ razorpaySubscriptionId: subscriptionId });
        if (user) {
            const pricing = await (0, pricing_controller_1.getPricingConfig)();
...
async function getAutoPayStatus(req, res) {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ error: "Not authenticated" });
...
async function getInvoiceByResume(req, res) {
    try {
        ...
        const invoice = await Invoice_1.default.findOne({ resumeId }).sort({ createdAt: -1 });
        if (!invoice)
