...
// ─────────────────────────────────────────────
async function enhanceSkillsWithAI(userId, currentSkills, experience, summary, options) {
    ...
    if (Array.isArray(currentSkills)) {
        ...
        if (currentSkills.includes("<ul>")) {
            ...
            if (ulMatch) {
                ...
                if (liMatches) {
                    existingSkills = liMatches.map((li) => li.replace(/<\/?li>/g, "").trim());
                }
            }
        }
        else if (currentSkills.includes(",")) {
            existingSkills = currentSkills.split(",").map((s) => s.trim());
        }
        else {
            existingSkills = [currentSkills];
        }
...
// ─────────────────────────────────────────────
function cleanBulletSymbols(data) {
    ...
    if (data.trainingPrograms && Array.isArray(data.trainingPrograms)) {
        data.trainingPrograms = data.trainingPrograms.map((item) => {
            if (item.description && typeof item.description === "string") {
                item.description = cleanText(item.description);
            }
            return item;
...
// ─────────────────────────────────────────────
function cleanPhoneNumbersInData(data) {
    if (!data || typeof data !== "object")
        return data;
    if (data.personal) {
...
// ─────────────────────────────────────────────
function deepMergePreserveOriginal(original, enhanced) {
    ...
    for (const key of Object.keys(enhanced)) {
        ...
        if (enhVal === null || enhVal === undefined)
            continue;
        if (typeof enhVal === "object" &&
...
// ─────────────────────────────────────────────
function postEnhancementCleanup(data) {
    if (!data || typeof data !== "object")
        return data;
...
async function enhanceResume(req, res) {
    try {
        ...
        const latestVersion = await ResumeVersion_1.default.findOne({
            resumeId: new mongoose_1.default.Types.ObjectId(id),
        }).sort({ createdAt: -1 });
        ...
        const { skills: enhancedSkills, usage: skillsUsage } = await enhanceSkillsWithAI(userId, currentData.skills, currentData.experience, currentData.summary, { skipLogging: true, guestId });
        ...
        // STEP 8: Normalize
        const normalized = (0, normalizeParsedResume_1.normalizeParsedResume)(cleanedData3);
        // STEP 9: Deep merge
        const mergedData = deepMergePreserveOriginal(currentData, normalized);
        // Save new version
        const newVersion = new ResumeVersion_1.default({
            resumeId: resume._id,
            resume: resume._id,
            data: mergedData,
        });
