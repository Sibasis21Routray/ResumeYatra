    ...
    if (source.skills) {
        if (typeof source.skills === 'string') {
            ...
            if (skillsList.length > 0) {
                normalized.skills = '<ul>' + skillsList.map((s) => `<li>${s}</li>`).join('') + '</ul>';
            }
        }
            ...
            normalized.academicProjects = projectArray
                ...
                if (project.technologies) {
                    if (Array.isArray(project.technologies)) {
                        proj.technologies = project.technologies.filter((t) => t && typeof t === 'string');
                    }
            ...
            normalized.clientProjects = clientArray
                ...
                if (item.toolsTechnologies) {
                    if (Array.isArray(item.toolsTechnologies)) {
                        project.toolsTechnologies = item.toolsTechnologies.filter((t) => t);
                    }
