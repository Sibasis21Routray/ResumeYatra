...
exports.suggestSkillsByCategories = suggestSkillsByCategories;
...
const spellCheck_1 = require("../utils/spellCheck");
const TokenUsage_1 = __importDefault(require("../models/TokenUsage"));
...
const openai = new openai_1.default({
    apiKey: api_1.default.openaiApiKey,
});
// Context-specific prompt builders
function summaryPromptBuilder(text, numSuggestions, metadata) {
    const { totalYears, experienceSummary, keyAchievements, experienceCount, techStack, } = metadata || {};
    const prompt = `Generate exactly 1 professional resume summary paragraph for a candidate with the following background:

Experience: ${totalYears || 0} years total experience${experienceSummary ? `, including: ${experienceSummary}` : ""}
...
Generate a first-person summary that incorporates the candidate's actual experience and achievements.`;
    const systemMessage = "You are a professional resume writing expert. Generate exactly 1 first-person summary paragraph (using 'I') that reflects the candidate's actual experience background provided. Return ONLY the paragraph text, no bullet points or formatting.";
    return { prompt, systemMessage };
}
function bulletPromptBuilder(context, text, numSuggestions, metadata) {
    let contextSpecific = "";
    let requirements = "";
    ...
    if (context === "experience") {
        ...
        if (hasContent) {
            contextSpecific = `Generate ${numSuggestions} professional bullet points to enhance this work experience${title ? ` as ${title}` : ""}${company ? ` at ${company}` : ""}${keywords && keywords.length > 0
                ? `. Focus on skills and achievements related to: ${keywords.join(", ")}`
                : ""}.`;
        }
        else {
            contextSpecific = `Generate ${numSuggestions} professional bullet points for a work experience${title ? ` as ${title}` : ""}${company ? ` at ${company}` : ""}${keywords && keywords.length > 0
...
- Use professional resume language and terminology`;
        ...
        if (hasContent) {
            contextSpecific = `Generate ${numSuggestions} professional bullet points to enhance this project description${name ? ` for ${name}` : ""}${technologies ? ` using ${technologies}` : ""}${keywords && keywords.length > 0
                ? `. Focus on features including: ${keywords.join(", ")}`
                : ""}.`;
        }
        else {
            const techList = technologies || "React and Node.js";
            const techArray = techList.split(",").map((t) => t.trim());
            ...
            contextSpecific = `Generate ${numSuggestions} bullet points for a resume project section about "${name || "this project"}" built with ${techList}.

Examples:
- Developed "${name || "this project"}" using ${tech1} and ${tech2}, implementing user authentication and data management features
- Built RESTful APIs for "${name || "this project"}" using ${tech3}, enabling seamless frontend-backend communication
- Designed database schemas for "${name || "this project"}" with ${tech4}, optimizing data storage and retrieval
- Created responsive UI components for "${name || "this project"}" using ${tech5}, enhancing user experience across devices
...
Return ONLY the bullet points, no other text or explanations.`;
    const systemMessage = `You are a professional resume writing expert. Generate exactly ${numSuggestions} ${context === "skills" ? "skill words" : "bullet points"} in bullet format. Return ONLY the bullet points.`;
    return { prompt, systemMessage };
}
...
function processBulletResponse(responseText) {
...
async function getAutoSuggestions(userId, text, context, metadata, numSuggestions) {
    if (!api_1.default.openaiApiKey) {
    ...
    try {
        ...
        if (context === "skills") {
            ...
            const categorizedSkills = await suggestSkillsByCategories(userId, jobTitle);
            // Format as HTML with categories
            const htmlFormatted = categorizedSkills
                .map((group) => `<h3>${group.category}</h3><ul>${group.skills
            ...
            if (context === "summary") {
                ...
                promptData = bulletPromptBuilder(context, text, suggestionsCount, metadata);
            }
            ...
            if (userId && usage) {
                await TokenUsage_1.default.create({
                    userId,
                    ...
                    action: `getAutoSuggestions_${context}`,
                });
            }
        ...
        const qualityChecked = validSuggestions
            .map((suggestion) => {
            const { corrected } = (0, spellCheck_1.spellCheckContent)(suggestion);
            return corrected;
        })
...
async function enhanceText(userId, text, tone = "professional", maxWords, format = "bullets") {
    // For paragraphs, use summary context
    if (format === "paragraphs") {
        const suggestions = await getAutoSuggestions(userId, text, "summary", undefined, 1);
        return suggestions[0] || text;
    }
    // For bullets, use experience context as default
    const suggestions = await getAutoSuggestions(userId, text, "experience", undefined, 4);
    return suggestions.map((s) => `- ${s}`).join("\n");
}
async function suggestImprovements(text) {
    // This function is not directly related to the core requirement, return empty for now
    return [];
}
async function suggestSkills(userId, summary) {
    return await getAutoSuggestions(userId, summary, "skills", undefined, 6);
}
async function suggestSkillsByCategories(userId, jobTitle, industry, experienceData) {
    if (!api_1.default.openaiApiKey) {
        throw new Error("OpenAI API key not configured. Set OPENAI_API_KEY in environment.");
    }
    const categories = [
    ...
    for (const category of categories) {
        ...
        const prompt = `Based on the job title "${jobTitle}"${industry ? ` in the ${industry} industry` : ""}, suggest exactly 5 SHORT relevant ${categorySpecificPrompt} that would be valuable for this role.
        ...
        try {
            ...
            if (userId && usage) {
                await TokenUsage_1.default.create({
                    ...
                    completionTokens: usage.completion_tokens ?? usage.completionTokens ?? usage.output_tokens ?? usage.outputTokens ?? 0,
                    totalTokens: usage.total_tokens ?? usage.totalTokens ?? 0,
                    aiModel: completion.model || "gpt-5.2",
                    action: `suggestSkillsByCategories_${category}`,
                });
            }
            let responseText = completion.choices[0]?.message?.content || "";
            // Clean up any markdown or formatting
            ...
            for (const line of lines) {
                ...
                if (line.includes("Core / Technical Skills") ||
            ...
            while (finalSkills.length < 5) {
                ...
                const fallbackSkill = fallbacks[category][finalSkills.length];
...
async function suggestHobbies(jobTitle, industry, existingHobbies) {
    // Simplified implementation
    return ["Reading", "Travel", "Sports", "Music", "Cooking"];
}
async function suggestDescriptionParagraphs(userId, context, currentDescription = "", metadata) {
    return await getAutoSuggestions(userId, currentDescription, context, metadata, 4);
}
async function suggestSummaryParagraphs(userId, currentSummary = "", jobTitle, industry, keywords, experienceSummary, projectSummary, experienceCount = 0) {
    return await getAutoSuggestions(userId, currentSummary, "summary", undefined, 1);
}
async function suggestKeyAchievements(userId, jobTitle, industry, existingAchievements, metadata) {
    return await getAutoSuggestions(userId, jobTitle || "", "experience", undefined, 4);
}
