...
function fixSkillsExtraction(data) {
    ...
    if (Array.isArray(data.skills)) {
        const cleanedSkills = data.skills
            ...
            .filter((skill) => skill && skill !== '' && skill !== 'undefined')
            .filter((skill, index, self) => self.findIndex((s) => s.toLowerCase() === skill.toLowerCase()) === index);
...
function fixEducationDates(data) {
    ...
    data.education = data.education.map((edu) => {
        if (edu.startDate && typeof edu.startDate === 'string' && edu.startDate.includes('-')) {
            const [start, end] = edu.startDate.split('-').map((s) => s.trim());
            edu.startDate = start;
        ...
        if (edu.graduationDate && typeof edu.graduationDate === 'string' && edu.graduationDate.includes('-') && (!edu.startDate || edu.startDate === '')) {
            const [start, end] = edu.graduationDate.split('-').map((s) => s.trim());
            edu.startDate = start;
            edu.graduationDate = end;
        }
        delete edu.endDate;
...
function cleanupEducation(data) {
    ...
    data.education.sort((a, b) => (b.startDate || '0').localeCompare(a.startDate || '0'));
    return data;
}
function fixCertifications(data) {
    ...
    if (certsToMove.length > 0) {
        ...
        for (const cert of certsToMove) {
            const exists = data.certifications.some((c) => c.name && c.name.toLowerCase() === cert.name.toLowerCase());
            if (!exists) {
...
function fixSocialProfiles(data) {
    ...
    data.socialProfiles = data.socialProfiles
        ...
        .filter((profile) => profile.platform && profile.platform !== 'undefined');
    return data;
}
...
function cleanBulletPointsFromDescriptions(data) {
    ...
    if (data.academicProjects && Array.isArray(data.academicProjects)) {
        data.academicProjects = data.academicProjects.map((project) => {
            if (project.description && typeof project.description === 'string') {
                const lines = project.description.split(/\r?\n/);
                const cleanedLines = lines.map(line => {
                    let cleaned = line.trim();
...
async function extractDataFromText(userId, text, guestId = null) {
...
async function parseResume(userId, filePath, guestId = null) {
    console.log("🚀 Starting resume parsing pipeline (Text-based)...");
    console.log(`📂 File: ${filePath}`);
    // Check cache
    const cacheKey = `${filePath}-${fs_1.default.statSync(filePath).mtimeMs}`;
    ...
    const result = (0, normalizeParsedResume_1.normalizeParsedResume)(processedData);
