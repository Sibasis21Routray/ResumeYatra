...
exports.buildBlastoiseTemplate = buildBlastoiseTemplate;
function buildBlastoiseTemplate(data, theme) {
    const defaultTheme = {
        primary: '#0d3b66',
        secondary: '#7f8c8d',
        background: '#ffffff',
    ...
    const userFontFamily = data.formatting?.fontFamily || data.fontFamily || 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    ...
    if (data.personal?.email)
        contactItems.push(data.personal.email);
    if (data.personal?.phone)
        contactItems.push(data.personal.phone);
    if (data.personal?.alternatePhone)
        contactItems.push(data.personal.alternatePhone);
    if (data.personal?.location)
        contactItems.push(data.personal.location);
    if (data.personal?.country)
        contactItems.push(data.personal.country);
    if (data.personal?.pinCode)
        contactItems.push(data.personal.pinCode);
    if (data.personal?.fullAddress)
        contactItems.push(data.personal.fullAddress);
    if (data.personal?.linkedinUrl)
        contactItems.push(`<a href="${data.personal.linkedinUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Linkedin: ${data.personal.linkedinUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.githubUrl)
        contactItems.push(`<a href="${data.personal.githubUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">GitHub: ${data.personal.githubUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.portfolioUrl)
        contactItems.push(`<a href="${data.personal.portfolioUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Portfolio: ${data.personal.portfolioUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.website)
        contactItems.push(`<a href="${data.personal.website}" target="_blank" style="color: ${textColor}; text-decoration: none;">Website: ${data.personal.website.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.twitterUrl)
        contactItems.push(`<a href="${data.personal.twitterUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Twitter: ${data.personal.twitterUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.facebookUrl)
        contactItems.push(`<a href="${data.personal.facebookUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Facebook: ${data.personal.facebookUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.instagramUrl)
        contactItems.push(`<a href="${data.personal.instagramUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Instagram: ${data.personal.instagramUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.behanceUrl)
        contactItems.push(`<a href="${data.personal.behanceUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Behance: ${data.personal.behanceUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.dribbbleUrl)
        contactItems.push(`<a href="${data.personal.dribbbleUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Dribbble: ${data.personal.dribbbleUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.stackoverflowUrl)
        contactItems.push(`<a href="${data.personal.stackoverflowUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Stack Overflow: ${data.personal.stackoverflowUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.mediumUrl)
        contactItems.push(`<a href="${data.personal.mediumUrl}" target="_blank" style="color: ${textColor}; text-decoration: none;">Medium: ${data.personal.mediumUrl.replace('https://', '').replace('http://', '')}</a>`);
    if (data.personal?.fathersName)
        contactItems.push(`Father's Name: ${data.personal.fathersName}`);
    if (data.personal?.dob)
        contactItems.push(`DOB: ${data.personal.dob}`);
  ...
  <div class="container">
    <div class="left-column">
        ...
        ${data.experience.map((e, index) => `
          <div class="exp-item" data-section="experience" data-index="${index}">
            <div class="exp-header">
              ...
              <div class="exp-date">${e.startDate || ''} - ${e.endDate || 'Present'}</div>
            </div>
            ${e.description ? `<p class="exp-description">${e.description}</p>` : ''}
            ${(Array.isArray(e.achievements) ? e.achievements : (e.achievements || '').split('\n')).filter((achievement) => achievement && (typeof achievement === 'string' ? achievement.trim() : String(achievement).trim())).length > 0 ? `
              <ul class="exp-achievements">
                ${(Array.isArray(e.achievements) ? e.achievements : (e.achievements || '').split('\n')).filter((achievement) => achievement && (typeof achievement === 'string' ? achievement.trim() : String(achievement).trim())).map((achievement, achIndex) => `
      ...
      <div class="main-section">
        ...
        ${data.education.filter((e) => e && (e.degree || e.school)).map((e, index) => `
          <div class="exp-item" data-section="education" data-index="${index}">
            <div class="exp-header">
              <div class="exp-title">${e.degree || ''}${e.qualification ? ` (${e.qualification})` : ''}</div>
              <div class="exp-company">${e.school || ''}${e.field ? ` - ${e.field}` : ''}${e.location ? `, ${e.location}` : ''}</div>
              <div class="exp-date">${e.graduationDate || ''}</div>
            </div>
            ${e.description ? `<p class="exp-description">${e.description}</p>` : ''}
            ${e.achievements && e.achievements.length > 0 ? `
              <ul class="exp-achievements">
                ${e.achievements.filter((achievement) => achievement && achievement.trim()).map((achievement, achIndex) => `
      ...
      <div class="main-section">
        ...
        ${data.certifications.filter((cert) => cert && (cert.name || cert.issuer)).map((cert, index) => `
        ...
        ${data.projects.map((project, index) => `
          <div class="exp-item" data-section="projects" data-index="${index}">
            <div class="exp-header">
              ...
              <div class="exp-company">${project.description ? project.description : ''}</div>
              <div class="exp-date">${project.startDate || ''} - ${project.endDate || 'Present'}</div>
            </div>
            ${project.technologies ? `<p class="exp-description">Technologies: ${project.technologies}</p>` : ''}
            ${project.achievements && project.achievements.length > 0 ? `
              <ul class="exp-achievements">
                ${project.achievements.filter((achievement) => achievement && achievement.trim()).map((achievement, achIndex) => `
      ...
      <div class="main-section">
        ...
        <ul class="exp-achievements">
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, index) => `
            <li data-section="customSections" data-index="${index}">${entry.language || entry.title || entry.organization || 'Language'}${entry.level ? ` (${entry.level})` : ''}</li>
          `).join('') : '<li style="color: #7f8c8d; font-style: italic;">No languages added</li>'}
        </ul>
      </div>`;
        }
      ...
      <div class="main-section">
        ...
        <ul class="hobbies-list">
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, index) => `
            <li class="hobby-item" data-section="customSections" data-index="${index}">${entry.title || entry.organization || entry.description || 'Hobby'}</li>
          `).join('') : '<li style="color: #7f8c8d; font-style: italic;">No hobbies added</li>'}
        </ul>
      </div>`;
        }
      ...
      <div class="main-section">
        ...
        <ul class="exp-achievements">
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, index) => `
            <li data-section="customSections" data-index="${index}">${entry.title || entry.organization || entry.description || 'Achievement'}</li>
          `).join('') : '<li style="color: #7f8c8d; font-style: italic;">No achievements added</li>'}
        </ul>
      </div>`;
        }
      ...
      <div class="main-section">
        ...
        <ul class="exp-achievements">
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, index) => `
            <li data-section="customSections" data-index="${index}">${entry.title || entry.organization || entry.description || 'Responsibility'}</li>
          `).join('') : '<li style="color: #7f8c8d; font-style: italic;">No responsibilities added</li>'}
        </ul>
      </div>`;
        }
      ...
      <div class="main-section">
        ...
        <ul class="exp-achievements">
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry) => entry.isVisible).map((entry, index) => `
            <li data-section="customSections" data-index="${index}">${entry.title || entry.organization || entry.description || 'Tool'}</li>
          `).join('') : '<li style="color: #7f8c8d; font-style: italic;">No tools added</li>'}
        </ul>
      </div>`;
        }
        else {
      ...
      <div class="main-section">
        ...
        `).join('') : '<div style="color: #7f8c8d; font-style: italic;">No entries in this section</div>'}
      </div>`;
        }
    }).join('') : ""}
    </div>
    ...
    <div class="right-column">
      ...
      ${(data.sectionVisibility?.skills !== false && data.skills && (Array.isArray(data.skills) ? data.skills.filter((skill) => skill && (typeof skill === 'string' ? skill.trim() : skill)).length > 0 : (data.skills || '').split(',').filter((skill) => skill.trim()).length > 0)) ? `
      <div class="contact-section">
