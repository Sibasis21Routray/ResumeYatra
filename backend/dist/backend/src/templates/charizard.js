...
<div class="container">
  <div class="header" data-section="personal">
    ...
    <div class="header-contact-bar" data-section="personal">
      ...
      ${data.personal?.location || data.personal?.country || data.personal?.pinCode || data.personal?.fullAddress ? `<span class="contact-item" data-section="personal">${icons.location}${[data.personal?.location, data.personal?.country, data.personal?.pinCode, data.personal?.fullAddress].filter(Boolean).join(', ')}</span>` : ''}
      ...
      ${data.personal?.fathersName ? `<span class="contact-item" data-section="personal">Father's Name: ${data.personal.fathersName}</span>` : ''}
      ${data.personal?.dob ? `<span class="contact-item" data-section="personal">DOB: ${data.personal.dob}</span>` : ''}
      ${data.personal?.gender ? `<span class="contact-item" data-section="personal">Gender: ${data.personal.gender}</span>` : ''}
      ${data.personal?.maritalStatus ? `<span class="contact-item" data-section="personal">Marital Status: ${data.personal.maritalStatus}</span>` : ''}
    </div>
  </div>
    ...
    <div class="content-wrapper">
      ...
      ${(data.sectionVisibility?.projects !== false && data.projects && data.projects.length > 0) ? `<div class="section" data-section="projects">
        ...
        ${(data.projects || []).map((project, index) => `
          <div class="entry" data-section="projects" data-index="${index}">
            ...
            ${project.url ? `<div class="entry-content" data-section="projects" data-index="${index}" style="margin-top: 5px;">${icons.link}<a href="${project.url}" target="_blank" style="color: ${currentTheme.primary}; text-decoration: underline;">${project.urlText || project.url}</a></div>` : ''}
          </div>
        `).join('')}
      </div>` : ''}
      ...
      ${(data.sectionVisibility?.skills !== false && data.skills) ? `<div class="section" data-section="skills">
       ...
       ${(data.sectionVisibility?.languages !== false && data.languages && data.languages.length > 0) ? `<div class="section" data-section="languages">
        ...
        <div class="skills-list" data-section="languages">
          ${(data.languages || []).map((lang, index) => `<div class="skill-tag" data-section="languages" data-index="${index}">${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}</div>`).join('')}
        </div>
      </div>` : ''}
      ...
      ${(data.sectionVisibility?.hobbies !== false && data.hobbies && data.hobbies.length > 0) ? `<div class="section" data-section="hobbies">
        ...
        <div class="skills-list" data-section="hobbies">
          ${(data.hobbies || []).map((hobby, index) => `<div class="skill-tag" data-section="hobbies" data-index="${index}">${hobby}</div>`).join('')}
        </div>
      </div>` : ''}
      ...
      ${(data.sectionVisibility?.certifications !== false && data.certifications && data.certifications.length > 0) ? `<div class="section" data-section="certifications">
        ...
        ${(data.certifications || []).map((cert, index) => `
          <div class="entry" data-section="certifications" data-index="${index}">
            ...
            ${cert.url ? `<div class="entry-content" data-section="certifications" data-index="${index}" style="margin-top: 5px;">${icons.link}<a href="${cert.url}" target="_blank" style="color: ${currentTheme.primary}; text-decoration: underline;">View Certificate</a></div>` : ''}
          </div>
        `).join('')}
      </div>` : ''}
