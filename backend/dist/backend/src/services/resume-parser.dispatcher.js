...
const normalizeParsedResume_1 = require("../utils/normalizeParsedResume");
// DeepInfra configuration
const deepinfra = new openai_1.default({
...
async function convertDocxToPdf(filePath) {
    ...
    const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "docx-pdf-"));
    const outputDir = path_1.default.join(tempDir, "pdf");
    fs_1.default.mkdirSync(outputDir, { recursive: true });
    try {
        // Use LibreOffice to convert DOCX to PDF
        (0, child_process_1.execSync)(`libreoffice --headless --convert-to pdf --outdir "${outputDir}" "${filePath}"`, { timeout: 60000 });
        const files = fs_1.default.readdirSync(outputDir);
        const pdfFile = files.find((f) => f.endsWith(".pdf"));
        if (!pdfFile) {
            throw new Error("PDF not generated from DOCX");
        }
        const pdfPath = path_1.default.join(outputDir, pdfFile);
...
async function convertPDFToHighQualityImage(filePath) {
    console.log("🔄 Converting PDF to high-quality images using Poppler...");
    const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "resume-"));
    const outputPrefix = path_1.default.join(tempDir, "page");
    try {
        // 200 DPI is sufficient for OCR and faster than 400 DPI
        (0, child_process_1.execSync)(`pdftoppm -jpeg -jpegopt quality=95 -r 200 "${filePath}" "${outputPrefix}"`);
        ...
        const buffers = files.map((file, i) => {
            const buffer = fs_1.default.readFileSync(path_1.default.join(tempDir, file));
...
async function extractStructuredDataFromPDFImages(imageBuffers) {
...
6. Return ONLY valid JSON, no markdown formatting`,
            ...
            try {
                ...
                console.log(`✅ Unified extraction successful: ${JSON.stringify(parsedData).length} chars`);
            }
            catch (err) {
                console.warn("⚠️ AI response could not be parsed as JSON, returning raw text");
                parsedData = { rawText: responseContent };
            }
            return parsedData;
        }
        catch (err) {
            console.log(`⚠️ Unified extraction failed on attempt ${attempt + 1}: ${err.message}`);
            attempt++;
...
async function parseResumeFromPDF(filePath) {
    console.log("🚀 Starting PDF resume parsing pipeline (Vision-first, single API call)...");
    console.log(`📂 File: ${filePath}`);
    try {
        ...
        const result = (0, normalizeParsedResume_1.normalizeParsedResume)(aiParsed);
        console.log("🎉 FINAL PARSING RESULT READY");
        return result;
    }
    catch (error) {
        console.error("❌ Resume parsing failed:", error.message);
        return (0, normalizeParsedResume_1.normalizeParsedResume)({
            error: error.message,
            rawText: "",
        });
    }
