...
async function generateTxt(data) {
    ...
    if (data.personal && typeof data.personal === 'object') {
        if (data.personal.name)
            txt += `${stripHtml(String(data.personal.name))}\n`;
        if (data.personal.email)
            txt += `${stripHtml(String(data.personal.email))}\n`;
        if (data.personal.phone)
            txt += `${stripHtml(String(data.personal.phone))}\n`;
        if (data.personal.location)
            txt += `${stripHtml(String(data.personal.location))}\n`;
        if (data.personal.linkedin)
            txt += `${stripHtml(String(data.personal.linkedin))}\n`;
        if (data.personal.github)
            txt += `${stripHtml(String(data.personal.github))}\n`;
    }
    txt += '\n';
    // Summary
    ...
    if (Array.isArray(data.projects) && data.projects.length > 0) {
        ...
        data.projects.forEach((project) => {
            if (!project || typeof project !== 'object')
                return;
            if (project.name)
                txt += `${stripHtml(String(project.name))}\n`;
            if (project.technologies)
                txt += `Technologies: ${stripHtml(String(project.technologies))}\n`;
            if (project.description)
                txt += `${stripHtml(String(project.description))}\n`;
            if (project.url)
                txt += `${stripHtml(String(project.url))}\n`;
            txt += '\n';
        });
    }
    // Skills
