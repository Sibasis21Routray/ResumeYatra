...
async function getAutoSuggestions(text, context, metadata, numSuggestions) {
...
async function enhanceText(text, tone = "professional", maxWords, format = "bullets") {
    ...
    const suggestions = await getAutoSuggestions(text, "experience", undefined, 4);
...
async function suggestSkills(summary) {
    return await getAutoSuggestions(summary, "skills", undefined, 6);
}
async function suggestSkillsByCategories(jobTitle, industry, experienceData) {
    if (!api_1.default.openaiApiKey) {
        throw new Error("OpenAI API key not configured. Set OPENAI_API_KEY in environment.");
    }
    const categories = [
...
async function suggestDescriptionParagraphs(context, currentDescription = "", metadata) {
    return await getAutoSuggestions(currentDescription, context, metadata, 4);
}
async function suggestSummaryParagraphs(currentSummary = "", jobTitle, industry, keywords, experienceSummary, projectSummary, experienceCount = 0) {
    return await getAutoSuggestions(currentSummary, "summary", undefined, 1);
}
async function suggestKeyAchievements(jobTitle, industry, existingAchievements, metadata) {
