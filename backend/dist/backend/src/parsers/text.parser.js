...
exports.extractPersonalInfo = extractPersonalInfoImproved;
exports.parseSkillsImproved = parseSkillsImproved;
exports.parseExperienceImproved = parseExperienceImproved;
exports.parseEducationImproved = parseEducationImproved;
exports.parseProjectsImproved = parseProjectsImproved;
exports.parseLanguagesImproved = parseLanguagesImproved;
...
function extractSectionsImproved(text) {
    console.log("\n=== EXTRACTING SECTIONS ===");
    const sections = {};
    if (!text || typeof text !== "string") {
        return sections;
            ...
            for (const pattern of header.patterns) {
                ...
                if (pattern.test(trimmedLine)) {
                    ...
                    (trimmedLine === trimmedLine.toUpperCase() &&
                        ...
                        (i + 1 < lines.length &&
                            !sectionHeaders.some((h) => h.patterns.some((p) => p.test(lines[i + 1].trim()))));
                    if (isLikelyHeader) {
    ...
    if (currentSection && content.length > 0) {
        ...
        sections[currentSection] = existingContent
            ? existingContent + "\n" + newContent
            : newContent;
        console.log(`  Saved final section "${currentSection}" with ${content.length} lines`);
    }
    // If no sections were found with headers, try to guess based on content
    if (Object.keys(sections).length === 0) {
        console.log("No sections found with headers, using content-based guessing...");
        sections.personal = text;
    }
    console.log(`=== FINAL SECTIONS: ${Object.keys(sections).length} sections ===`);
...
function looksLikeExperienceBlock(line) {
    return (/(pvt\.?|ltd\.?|solutions|technologies)/i.test(line) ||
...
function parseSkillsImproved(text) {
    console.log("\n=== PARSING SKILLS ===");
    ...
    let cleanText = text
        .replace(/AREAS OF EXPERTISE|SKILLS|TECHNICAL SKILLS|CORE COMPETENCIES/gi, "")
        .replace(/\s+/g, " ")
        .trim();
    ...
    for (let candidate of skillCandidates) {
        ...
        if (nonSkills.some((ns) => lowerCandidate.includes(ns)))
            continue;
        // Fix specific issues
...
function parseExperienceImproved(text) {
    console.log("\n=== PARSING EXPERIENCE ===");
    const experience = [];
    if (!text)
        return experience;
...
function parseEducationImproved(text) {
    console.log("\n=== PARSING EDUCATION ===");
    const education = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.match(/\b(?:B\.?Tech|M\.?Tech|Bachelor|Master|MBA|PhD)\b/i)) {
            ...
            // Extract degree
            const degreeMatch = line.match(/(B\.?Tech|M\.?Tech|Bachelor|Master|MBA|PhD)/i);
            if (degreeMatch) {
...
function parseProjectsImproved(text) {
    console.log("\n=== PARSING PROJECTS ===");
    const projects = [];
...
function extractPersonalInfoImproved(text) {
    ...
    headerText = headerText.replace(/([a-z0-9._%+-]{3,30})(gmail\.com|yahoo\.com|outlook\.com)/gi, "$1@$2");
    ...
    const phoneMatch = text.match(/(\+?91[\s-]?)?[789]\d{9}/) || text.match(/\b\d{10}\b/);
    if (phoneMatch) {
    ...
    // ------------------------------
    const locationKeywords = /(india|odisha|delhi|mumbai|bangalore|hyderabad|pune|kolkata|chennai)/i;
    ...
    if (!personal.name && lines.length >= 2) {
        ...
        const allCapsName = firstTwo.every((l) => /^[A-Z\s]+$/.test(l) && // only caps + spaces
            !l.includes("@") &&
            !/\d/.test(l));
        if (allCapsName) {
