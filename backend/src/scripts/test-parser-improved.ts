...
Advanced Node.js – Frontend Masters`;
    ...
    // Use the text parser
    const parsed = parsePlainText(resumeText);
    ...
    parsed.experience.forEach((exp, i) => {
      ...
      if (exp.startDate) console.log(`     Duration: ${exp.startDate} - ${exp.endDate || 'Present'}`);
    });
    ...
    parsed.education.forEach((edu, i) => {
      ...
      if (edu.school) console.log(`     From: ${edu.school}`);
    });
