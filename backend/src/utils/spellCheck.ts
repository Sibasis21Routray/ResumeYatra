...
const GRAMMAR_RULES = [
  { pattern: /\b(a|an)\s+[aeiouAEIOU]\b/g, replacement: (match: string) => match.includes('a') ? match.replace('a', 'an') : match.replace('an', 'a') },
  { pattern: /\b(there|their|they're)\b/g, replacement: (match: string) => {
...
export function validateGrammar(text: string): string {
  let validatedText = text;
  
  // Apply grammar rules
  GRAMMAR_RULES.forEach(rule => {
    validatedText = validatedText.replace(rule.pattern, rule.replacement as any);
  });
  
  return validatedText;
}

export function enhanceWithActionVerbs(text: string): string {
...
export function spellCheckContent(text: string): { corrected: string; issues: string[] } {
  const issues: string[] = [];
  let corrected = text;
...
export function isContentQualityPoor(text: string): boolean {
  ...
  if (maxFreq > words.length * 0.3) return true; // More than 30% repetition
  
  return false;
}

export function retryWithQualityCheck(text: string, maxRetries: number = 2): string {
  let attempts = 0;
