export function normalizeParsedResume(parsed: any) {
  ...
  function parseDateRange(dates: any, startKey: string, endKey: string) {
    let startDate = null;
    let endDate = null;
    ...
    if (typeof dates === "string" && dates) {
      ...
      if (parts.length >= 2) endDate = parts[1];
  ...
  return {
    ...
    professionalContext: {
      ...
      functionalDomainCustom:
        parsed.professionalContext?.functionalDomainCustom || null,
      geographicScope: parsed.professionalContext?.geographicScope || null,
      revenueResponsibility:
        parsed.professionalContext?.revenueResponsibility || null,
    },
  };
}
