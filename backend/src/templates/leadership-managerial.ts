...
<div class="resume-container">
  ...
  <div class="section skills-section" data-section="skills">
    ...
    ${Array.isArray(data.skills) ? `
      <div class="skills-list">
        ${data.skills.slice(0, 15).map((skill: any, index: number) => `
  ...
  <div class="section" data-section="experience">
    ...
    ${sortedExperience.map((exp: any, index: number) => `
      <div class="experience-item" data-section="experience" data-index="${index}">
        ...
        ${exp.description ? `<ul>${exp.description.split('\n').filter((line: string) => line.trim()).map((line: string, lineIndex: number) => `<li>${line.trim()}</li>`).join('')}</ul>` : ''}
      </div>
    `).join('')}
  </div>` : ''}
  ...
  <div class="section" data-section="internships">
    ...
    ${data.internships.map((item: any, index: number) => `
      <div class="experience-item" data-section="internships" data-index="${index}">
        ...
        ${item.description ? `<ul>${item.description.split('\n').filter((line: string) => line.trim()).map((line: string, lineIndex: number) => `<li data-section="internships" data-index="${index}" data-item-index="${lineIndex}">${line.trim()}</li>`).join('')}</ul>` : ''}
      </div>
    `).join('')}
  </div>` : ''}
  ...
  <div class="section" data-section="additional">
    ...
    <p><strong>Languages:</strong> ${data.languages.map((lang: any, index: number) => `${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}`).join(', ')}</p>
    ` : ''}
