  ...
  <div class="main-grid">
    <div class="sidebar">
      ...
      <div class="sidebar-section" data-section="skills">
        ...
        ${Array.isArray(data.skills) ? data.skills.map((s: any, index: number) => `<div class="skill-item" data-section="skills" data-index="${index}">• ${s}</div>`).join('') : `<div class="skill-item" data-section="skills" data-index="0">${data.skills || ''}</div>`}
      </div>` : ''}
      ...
      <div class="sidebar-section" data-section="keyAchievements">
        ...
        ${data.keyAchievements.map((a: any, index: number) => `<div class="skill-item" data-section="keyAchievements" data-index="${index}">• ${a}</div>`).join('')}
      </div>` : ''}
      ...
      ${(data.sectionVisibility?.languages !== false && data.languages && data.languages.length > 0) ? `<div class="sidebar-section" data-section="languages">
        ...
        <div style="font-size: 9pt; color: rgba(22, 21, 21, 0.9);">${data.languages.map((lang: any) => `${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}`).join(', ')}</div>
      </div>` : ''}
      ...
      ${(data.sectionVisibility?.socialLinks !== false && data.socialLinks && data.socialLinks.length > 0) ? `<div class="sidebar-section" data-section="socialLinks">
        ...
        <div style="font-size: 9pt; color: rgba(46, 44, 44, 0.9);">${data.socialLinks.map((link: any, index: number) => `<a href="${link.url}" target="_blank" style="color: inherit; text-decoration: none;">${link.urlText || link.url.replace('https://', '').replace('http://', '')}</a>`).join(', ')}</div>
      </div>` : ''}
      ...
      ${hasResponsibilitiesContent ? `<div class="sidebar-section" data-section="responsibilities">
        ...
        <ul style="color: rgba(31, 30, 30, 0.9);">
          ${(Array.isArray(data.responsibilities) ? data.responsibilities : (data.responsibilities || '').split('\n')).filter((line: string) => line.trim()).map((line: string, index: number) => `<li style="font-size: 9pt;" data-section="responsibilities" data-index="${index}">${line.trim()}</li>`).join('')}
        </ul>
      </div>` : ''}
    ...
    <div class="content">
      ...
      <div class="main-section" data-section="experience">
        ...
        ${sortedExperience.map((exp: any, index: number) => `
          <div class="exp-item" data-section="experience" data-index="${index}">
            ...
            <ul>
              ${exp.description && typeof exp.description === 'string' ? exp.description.split('\n').filter((l:any) => l.trim()).map((l:any, lineIndex: number) => `<li data-section="experience" data-index="${index}" data-item-index="${lineIndex}">${l}</li>`).join('') : ''}
            </ul>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="careerObjective">
        <div class="section-title-main">CAREER OBJECTIVE</div>
        <p style="font-size: ${bodyFontSize}; color: var(--secondary-color);">${data.careerObjective}</p>
      </div>` : ''}
      ...
      <div class="main-section" data-section="internships">
        ...
        ${data.internships.map((item: any, index: number) => `
          <div class="exp-item" data-section="internships" data-index="${index}">
            <div class="exp-header" data-section="internships" data-index="${index}">
              <span>${item.title || ''}</span>
              <span>${item.startDate || ''} - ${item.endDate || ''}</span>
            </div>
            <div class="exp-subhead" data-section="internships" data-index="${index}">${item.company || ''}${item.location ? `, ${item.location}` : ''}</div>
            <ul>
              ${item.description ? (typeof item.description === 'string' ? item.description.split('\n').filter((l: any) => l.trim()).map((l: any, lineIndex: number) => `<li data-section="internships" data-index="${index}" data-item-index="${lineIndex}">${l}</li>`).join('') : `<li data-section="internships" data-index="${index}">${item.description}</li>`) : ''}
            </ul>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="academicProjects">
        ...
        ${data.academicProjects.map((item: any, index: number) => `
          <div class="exp-item" data-section="academicProjects" data-index="${index}">
            <div class="exp-header" data-section="academicProjects" data-index="${index}">
              <span>${item.name || item.title || ''}</span>
              <span>${item.duration || ''}</span>
            </div>
            <div class="exp-subhead" data-section="academicProjects" data-index="${index}">${item.institution || ''}${item.course ? ` | ${item.course}` : ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="academicProjects" data-index="${index}">${item.description || ''}</p>
            ${item.technologies && item.technologies.length > 0 ? `<div class="exp-subhead" data-section="academicProjects" data-index="${index}"><strong>Technologies:</strong> ${Array.isArray(item.technologies) ? item.technologies.join(', ') : item.technologies}</div>` : ''}
            ${item.url ? `<p style="font-size: 8pt;" data-section="academicProjects" data-index="${index}"><strong>URL:</strong> <a href="${item.url}" target="_blank">${item.url}</a></p>` : ''}
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="leadershipPositions">
        ...
        ${data.leadershipPositions.map((item: any, index: number) => `
          <div class="exp-item" data-section="leadershipPositions" data-index="${index}">
            <div class="exp-header" data-section="leadershipPositions" data-index="${index}">
              <span>${item.position || item.title || ''}</span>
              <span>${item.startDate || ''} - ${item.endDate || ''}</span>
            </div>
            <div class="exp-subhead" data-section="leadershipPositions" data-index="${index}">${item.organization || ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="leadershipPositions" data-index="${index}">${item.description || ''}</p>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="trainingPrograms">
        ...
        ${data.trainingPrograms.map((item: any, index: number) => `
          <div class="exp-item" data-section="trainingPrograms" data-index="${index}">
            <div class="exp-header" data-section="trainingPrograms" data-index="${index}">
              <span>${item.name || ''}</span>
              <span>${item.completionDate || ''}</span>
            </div>
            <div class="exp-subhead" data-section="trainingPrograms" data-index="${index}">${item.provider || item.organization || ''}${item.duration ? ` | ${item.duration}` : ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="trainingPrograms" data-index="${index}">${item.description || ''}</p>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="scholarships">
        ...
        ${data.scholarships.map((item: any, index: number) => `
          <div class="exp-item" data-section="scholarships" data-index="${index}">
            <div class="exp-header" data-section="scholarships" data-index="${index}">
              <span>${item.name || ''}</span>
              <span>${item.year || ''}</span>
            </div>
            <div class="exp-subhead" data-section="scholarships" data-index="${index}">${item.provider || item.organization || ''}${item.amount ? ` | ${item.amount}` : ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="scholarships" data-index="${index}">${item.description || ''}</p>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="coCurricular">
        ...
        ${data.coCurricular.map((item: any, index: number) => `
          <div class="exp-item" data-section="coCurricular" data-index="${index}">
            <div class="exp-header" data-section="coCurricular" data-index="${index}">
              <span>${item.activity || ''}</span>
              <span>${item.year || (item.startDate ? `${item.startDate} - ${item.endDate || ''}` : '')}</span>
            </div>
            <div class="exp-subhead" data-section="coCurricular" data-index="${index}">${item.organization || ''}${item.role ? ` | ${item.role}` : ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="coCurricular" data-index="${index}">${item.description || ''}</p>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div class="main-section" data-section="extracurricular">
        ...
        ${data.extracurricular.map((item: any, index: number) => `
          <div class="exp-item" data-section="extracurricular" data-index="${index}">
            <div class="exp-header" data-section="extracurricular" data-index="${index}">
              <span>${item.activity || ''}</span>
              <span>${item.year || (item.startDate ? `${item.startDate} - ${item.endDate || ''}` : '')}</span>
            </div>
            <div class="exp-subhead" data-section="extracurricular" data-index="${index}">${item.organization || ''}${item.role ? ` | ${item.role}` : ''}</div>
            <p style="font-size: ${bodyFontSize};" data-section="extracurricular" data-index="${index}">${item.description || ''}</p>
          </div>
        `).join('')}
      </div>` : ''}

      ${data.customSections && data.customSections.length > 0 ? data.customSections.filter((section: any) => section.isVisible && section.entries && section.entries.length > 0 && section.entries.some((entry: any) => entry.isVisible)).map((section: any, sectionIndex: number) => `
