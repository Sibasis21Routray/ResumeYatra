...
<div class="resume-container">
  ...
  <div class="main-section" data-section="skills">
    ...
    <div class="skills-grid">
      ${Array.isArray(data.skills) ? data.skills.map((skill: any, index: number) => `
  ...
  <div class="main-section" data-section="keyAchievements">
    ...
    <ul>
      ${data.keyAchievements.filter((a: string) => a.trim()).map((achievement: string, index: number) => `<li data-section="keyAchievements" data-index="${index}">${achievement}</li>`).join('')}
    </ul>
  </div>` : ''}
  ...
  <div class="main-section" data-section="responsibilities">
    ...
    <ul>
      ${Array.isArray(data.responsibilities) ? data.responsibilities.filter((r: string) => r.trim()).map((resp: string, index: number) => `<li data-section="responsibilities" data-index="${index}">${resp}</li>`).join('') : (data.responsibilities || '').split('\n').filter((l: string) => l.trim()).map((line: string, index: number) => `<li data-section="responsibilities" data-index="${index}">${line.trim()}</li>`).join('')}
    </ul>
  </div>` : ''}
  ...
  <div class="main-section" data-section="tools">
    ...
    <ul>
      ${Array.isArray(data.tools) ? data.tools.filter((t: string) => t.trim()).map((tool: string, index: number) => `<li data-section="tools" data-index="${index}">${tool}</li>`).join('') : (data.tools || '').split('\n').filter((l: string) => l.trim()).map((line: string, index: number) => `<li data-section="tools" data-index="${index}">${line.trim()}</li>`).join('')}
    </ul>
  </div>` : ''}
  ...
  ${data.customSections && data.customSections.length > 0 ? data.customSections.filter((s: any) => s.isVisible).map((section: any) => `
    <div class="main-section">
      <div class="section-title">${section.heading || 'Additional'}</div>
      ${section.entries && section.entries.length > 0 ? section.entries.filter((e: any) => e.isVisible).map((entry: any) => `
        <div class="item-row">
