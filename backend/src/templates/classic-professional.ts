  ...
  <div class="main-body">
    <div class="sidebar">
      ...
      <div class="sidebar-section sidebar-mid" data-section="skills">
        ...
        ${Array.isArray(data.skills) ? data.skills.slice(0, 5).map((skill: any, index: number) => `
...
<div class="main-section" style="margin-bottom: 30px;" data-section="summary">
      ...
      <div class="main-section" data-section="experience">
        ...
        ${sortedExperience.map((exp: any, index: number) => `
          <div class="exp-item" data-section="experience" data-index="${index}">
            ...
            <ul>
              ${exp.description?.split('\n').slice(1).filter((l: string) => l.trim()).map((line: string, lineIndex: number) => `<li data-section="experience" data-index="${index}" data-item-index="${lineIndex}">${line.trim()}</li>`).join('')}
            </ul>
          </div>
        `).join('')}
      </div>
      ...
      ${(data.sectionVisibility?.languages !== false && data.languages && data.languages.length > 0) ? `<div class="main-section" data-section="languages">
        ...
        <p style="font-size: 9pt;">${data.languages.map((lang: any) => `${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}`).join(', ')}</p>
      </div>` : ''}
      ...
      ${data.keyAchievements && data.keyAchievements.length > 0 ? `<div class="main-section" data-section="keyAchievements">
        ...
        <ul>
          ${data.keyAchievements.map((achievement: string, index: number) => `<li style="font-size: 9pt;" data-section="keyAchievements" data-index="${index}">${achievement}</li>`).join('')}
        </ul>
      </div>` : ''}
