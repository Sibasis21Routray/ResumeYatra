  ...
  <div class="resume-container">
    <div class="sidebar">
      ...
      <div class="sidebar-section" data-section="skills">
        ...
        <ul class="skill-list">
          ${(Array.isArray(data.skills) ? data.skills : data.skills.split(',')).map((s: any, index: number) => `<li data-section="skills" data-index="${index}">${typeof s === 'string' ? s.trim() : s}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      <div class="sidebar-section" data-section="languages">
        ...
        <ul class="skill-list">
          ${data.languages.map((lang: any, index: number) => `<li data-section="languages" data-index="${index}">${lang.language || lang}${lang.level ? ` (${lang.level})` : ''}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      <div class="sidebar-section" data-section="hobbies">
        ...
        <ul class="skill-list">
          ${data.hobbies.map((hobby: string, index: number) => `<li data-section="hobbies" data-index="${index}">${hobby}</li>`).join('')}
        </ul>
      </div>` : ''}
    ...
    <div class="main-content">
      ...
      <div class="main-section" data-section="internships">
        ...
        ${data.internships.map((item: any, index: number) => `
          <div class="entry" data-section="internships" data-index="${index}">
            <div class="entry-date">${item.startDate || ''} - <br>${item.endDate || ''}</div>
            <div class="entry-details">
              <div class="entry-header">${item.title || ''}</div>
              <div class="entry-sub">${item.company || ''}, ${item.location || ''}</div>
        ...
        ${data.academicProjects.map((item: any, index: number) => `
          <div class="entry" data-section="academicProjects" data-index="${index}">
            ...
            <div class="entry-details">
              ...
              <div class="entry-desc">${item.description || ''}</div>
              ${item.technologies ? `<div class="entry-desc"><strong>Technologies:</strong> ${item.technologies}</div>` : ''}
            </div>
          </div>
        `).join('')}
      </div>` : ''}
        ...
        ${data.trainingPrograms.map((item: any, index: number) => `
          <div class="entry" data-section="trainingPrograms" data-index="${index}">
            ...
            <div class="entry-details">
              ...
              <div class="entry-sub">${item.provider || item.organization || ''}${item.duration ? ` | ${item.duration}` : ''}</div>
              <div class="entry-desc">${item.description || ''}</div>
            </div>
          </div>
        `).join('')}
      ...
      <div class="main-section" data-section="scholarships">
        ...
        ${data.scholarships.map((item: any, index: number) => `
          <div class="entry" data-section="scholarships" data-index="${index}">
            ...
            <div class="entry-details">
              <div class="entry-header">${item.name || ''}</div>
              <div class="entry-sub">${item.provider || item.organization || ''}${item.amount ? ` | ${item.amount}` : ''}</div>
              <div class="entry-desc">${item.description || ''}</div>
            </div>
          </div>
        `).join('')}
      ...
      <div class="main-section" data-section="coCurricular">
        ...
        ${data.coCurricular.map((item: any, index: number) => `
          <div class="entry" data-section="coCurricular" data-index="${index}">
            <div class="entry-date">${item.year || (item.startDate ? `${item.startDate} - ${item.endDate || ''}` : '')}</div>
            <div class="entry-details">
              <div class="entry-header">${item.activity || ''}</div>
              <div class="entry-sub">${item.organization || ''}${item.role ? ` | ${item.role}` : ''}</div>
              <div class="entry-desc">${item.description || ''}</div>
            </div>
          </div>
        `).join('')}
      ...
      <div class="main-section" data-section="tools">
        ...
        <ul style="font-size: ${userFontSize}px; line-height: 1.5; padding-left: 18px;">
          ${Array.isArray(data.tools) ? data.tools.map((tool: string, index: number) => `<li data-section="tools" data-index="${index}">${tool}</li>`).join('') : data.tools.split(',').map((tool: string, index: number) => `<li data-section="tools" data-index="${index}">${tool.trim()}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      ${data.customSections && data.customSections.length > 0 ? data.customSections.filter((section: any) => section.entries && section.entries.length > 0).map((section: any, sectionIndex: number) => `
