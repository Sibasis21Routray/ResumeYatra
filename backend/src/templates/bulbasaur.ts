    ...
    <div class="content-grid">
      ...
      <div class="left-column">
        ...
        ${(data.sectionVisibility?.education !== false && data.education && data.education.length > 0) ? `<div class="section" data-section="education">
          ...
          ${(data.education || []).filter((edu: any) => edu.degree || edu.qualification || edu.field || edu.school || edu.graduationDate || edu.description).map((edu: any, index: number) => `
        ...
        ${(data.sectionVisibility?.skills !== false && data.skills && (Array.isArray(data.skills) ? data.skills.filter((skill: any) => skill && (typeof skill === 'string' ? skill.trim() : skill)) : (data.skills || '').split(',').filter((skill: string) => skill.trim())).length > 0) ? `<div class="section" data-section="skills">
          <div class="section-title" data-section="skills">Areas of Expertise</div>
          <ul data-section="skills">
            ${(Array.isArray(data.skills) ? data.skills : (data.skills || '').split(',')).filter((skill: any) => skill && (typeof skill === 'string' ? skill.trim() : skill)).map((skill: any, index: number) => `
      ...
      <div class="right-column">
        ...
        ${(data.sectionVisibility?.experience !== false && data.experience && data.experience.length > 0) ? `<div class="section" data-section="experience">
          ...
          ${(data.experience || []).filter((exp: any) => exp.title || exp.company || exp.description || exp.startDate || exp.endDate).map((exp: any, index: number) => `
        ...
        ${(data.sectionVisibility?.projects !== false && data.projects && data.projects.length > 0) ? `<div class="section" data-section="projects">
          ...
          ${(data.projects || []).filter((project: any) => project.name || project.technologies || project.description || project.startDate || project.endDate).map((project: any, index: number) => `
        ...
        ${(data.sectionVisibility?.certifications !== false && data.certifications && data.certifications.length > 0) ? `<div class="section" data-section="certifications">
          ...
          ${(data.certifications || []).filter((cert: any) => cert.name || cert.issuer || cert.date || cert.url).map((cert: any, index: number) => `
        ...
        ${(data.internships && data.internships.length > 0) ? `<div class="section" data-section="internships">
          ...
          ${(data.internships || []).filter((item: any) => item.title || item.company || item.description || item.startDate || item.endDate).map((item: any, index: number) => `
        ...
        ${(data.academicProjects && data.academicProjects.length > 0) ? `<div class="section" data-section="academicProjects">
          ...
          ${(data.academicProjects || []).filter((item: any) => item.name || item.title || item.institution || item.description || item.duration).map((item: any, index: number) => `
            <div class="entry" data-section="academicProjects" data-index="${index}">
              ...
              ${item.technologies ? `<div style="margin-top:5px; font-size: ${Math.round(baseFontSize * 0.9)}px;"><strong>Technologies:</strong> ${item.technologies}</div>` : ''}
            </div>
          `).join('')}
        </div>` : ''}
        ...
        ${(data.leadershipPositions && data.leadershipPositions.length > 0) ? `<div class="section" data-section="leadershipPositions">
          ...
          ${(data.leadershipPositions || []).filter((item: any) => item.position || item.title || item.organization || item.description || item.startDate || item.endDate).map((item: any, index: number) => `
        ...
        ${(data.trainingPrograms && data.trainingPrograms.length > 0) ? `<div class="section" data-section="trainingPrograms">
          ...
          ${(data.trainingPrograms || []).filter((item: any) => item.name || item.provider || item.organization || item.description || item.completionDate || item.duration).map((item: any, index: number) => `
        ...
        ${(data.scholarships && data.scholarships.length > 0) ? `<div class="section" data-section="scholarships">
          ...
          ${(data.scholarships || []).filter((item: any) => item.name || item.provider || item.organization || item.description || item.year || item.amount).map((item: any, index: number) => `
        ...
        ${(data.coCurricular && data.coCurricular.length > 0) ? `<div class="section" data-section="coCurricular">
          ...
          ${(data.coCurricular || []).filter((item: any) => item.activity || item.organization || item.description || item.year || item.startDate || item.endDate).map((item: any, index: number) => `
        ...
        ${(data.extracurricular && data.extracurricular.length > 0) ? `<div class="section" data-section="extracurricular">
          ...
          ${(data.extracurricular || []).filter((item: any) => item.activity || item.organization || item.description || item.year || item.startDate || item.endDate).map((item: any, index: number) => `
        ...
        ${data.keyAchievements && data.keyAchievements.filter((a: string) => a && a.trim()).length > 0 ? `<div class="section" data-section="keyAchievements">
          <div class="section-title" data-section="keyAchievements">Key Achievements</div>
          <ul data-section="keyAchievements">
            ${(data.keyAchievements || []).filter((achievement: string) => achievement && achievement.trim()).map((achievement: string, index: number) => `<li data-section="keyAchievements" data-index="${index}">${achievement}</li>`).join('')}
          </ul>
        </div>` : ''}


        ${data.responsibilities && (Array.isArray(data.responsibilities) ? data.responsibilities : (data.responsibilities || '').split('\n')).filter((line: string) => line && line.trim()).length > 0 ? `<div class="section" data-section="responsibilities">
          <div class="section-title" data-section="responsibilities">Key Responsibilities</div>
          <ul data-section="responsibilities">
            ${(Array.isArray(data.responsibilities) ? data.responsibilities : (data.responsibilities || '').split('\n')).filter((line: string) => line && line.trim()).map((line: string, index: number) => `<li data-section="responsibilities" data-index="${index}">${line.trim()}</li>`).join('')}
          </ul>
        </div>` : ''}

        ${data.tools && (Array.isArray(data.tools) ? data.tools : (data.tools || '').split('\n')).filter((line: string) => line && line.trim()).length > 0 ? `<div class="section" data-section="tools">
          <div class="section-title" data-section="tools">Tools & Technologies</div>
          <ul data-section="tools">
            ${(Array.isArray(data.tools) ? data.tools : (data.tools || '').split('\n')).filter((line: string) => line && line.trim()).map((line: string, index: number) => `<li data-section="tools" data-index="${index}">${line.trim()}</li>`).join('')}
          </ul>
        </div>` : ''}


        ${data.customSections && data.customSections.length > 0 ? data.customSections.filter((section: any) => section.isVisible && section.entries && section.entries.some((entry: any) => entry.isVisible && (entry.title || entry.organization || entry.description))).map((section: any) => `
        <div class="section" data-section="customSections">
          ...
          ${section.entries && section.entries.length > 0 ? section.entries.filter((entry: any) => entry.isVisible && (entry.title || entry.organization || entry.description)).map((entry: any, entryIndex: number) => `
