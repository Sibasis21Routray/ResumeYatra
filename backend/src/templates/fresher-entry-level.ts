...
<div class="resume-wrapper">
  ...
  ${(Array.isArray(data.skills) ? data.skills : (data.skills || '').split(',')).filter((s: any) => s && (typeof s === 'string' ? s.trim() : s)).length > 0 ? `
    ...
    ${data.internships.map((item: any, index: number) => `
      <div class="experience-item" data-section="internships" data-index="${index}">
        <div class="item-row">
          ...
          <span class="date-text">${item.startDate || ''} – ${item.endDate || ''}</span>
        </div>
        <div class="job-title">${item.company || ''}${item.location ? `, ${item.location}` : ''}</div>
        ${item.description ? `<ul><li>${item.description.split('\n').filter((l: string) => l.trim()).map((line: string, lineIndex: number) => `${line.trim()}`).join('</li><li>')}</li></ul>` : ''}
      </div>
    `).join('')}
  </div>` : ''}
    ...
    ${data.academicProjects.map((item: any, index: number) => `
      <div class="experience-item" data-section="academicProjects" data-index="${index}">
        <div class="item-row">
          ...
          <span class="date-text">${item.duration || ''}</span>
        </div>
        <div class="job-title">${item.institution || ''}${item.course ? ` | ${item.course}` : ''}</div>
        ${item.description ? `<div style="font-size: 9pt; margin-bottom: 10px;">${item.description}</div>` : ''}
        ${item.technologies && item.technologies.length > 0 ? `<div class="job-title" style="font-style: normal;">Technologies: ${Array.isArray(item.technologies) ? item.technologies.join(', ') : item.technologies}</div>` : ''}
        ${item.url ? `<div style="font-size: 9pt; margin-bottom: 10px;"><a href="${item.url}" target="_blank" style="color: var(--accent);">${item.url}</a></div>` : ''}
      </div>
    `).join('')}
  </div>` : ''}
    ...
    ${data.trainingPrograms.map((item: any, index: number) => `
      <div class="experience-item" data-section="trainingPrograms" data-index="${index}">
        <div class="item-row">
          ...
          <span class="date-text">${item.completionDate || ''}</span>
        </div>
  ...
  <div class="section" data-section="scholarships">
    ...
    ${data.scholarships.map((item: any, index: number) => `
      <div class="experience-item" data-section="scholarships" data-index="${index}">
        <div class="item-row">
          <span class="company-name">${item.name || ''}</span>
          <span class="date-text">${item.year || ''}</span>
        </div>
    ...
    ${data.coCurricular.map((item: any, index: number) => `
      <div class="experience-item" data-section="coCurricular" data-index="${index}">
        <div class="item-row">
          ...
          <span class="date-text">${item.year || (item.startDate ? `${item.startDate} – ${item.endDate || ''}` : '')}</span>
        </div>
  ...
  ${data.keyAchievements && data.keyAchievements.filter((a: string) => a && a.trim()).length > 0 ? `
  <div class="section" data-section="keyAchievements">
    <div class="section-title">Key Achievements</div>
    <hr class="section-divider">
    <div style="font-size: ${bodyFontSize};">
      <ul>
        ${data.keyAchievements.map((achievement: string) => `
          <li>${achievement}</li>
        `).join('')}
      </ul>
    </div>
  ...
  ${(Array.isArray(data.tools) ? data.tools : (data.tools || '').split(',')).filter((t: any) => t && (typeof t === 'string' ? t.trim() : t)).length > 0 ? `
  <div class="section" data-section="tools">
    <div class="section-title">Tools & Technologies</div>
    <hr class="section-divider">
    <div class="skills-grid">
      ${(Array.isArray(data.tools) ? data.tools : (data.tools || '').split(',')).map((tool: any) => `
  ...
  <div class="section" data-section="hobbies">
    ...
    <hr class="section-divider">
    <div style="font-size: ${bodyFontSize};">
      ${(Array.isArray(data.hobbies) ? data.hobbies : [data.hobbies]).map((hobby: any) => `
