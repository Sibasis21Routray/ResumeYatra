...
import { getPricingConfig } from '../controllers/pricing.controller';
import { uploadBuffer } from './cloudinary.service';

export async function generateInvoiceHtml(invoice: IInvoice, dynamicSignatureUrl?: string) {
  const logoPath = path.join(__dirname, '../../uploads/invoice_assets/logo.png');
  const signaturePath = path.join(__dirname, '../../uploads/invoice_assets/signature.png');
  ...
  try {
    ...
    if (!dynamicSignatureUrl && fs.existsSync(signaturePath)) {
...
function getItemDescription(type: string) {
  const [mainType, addon] = (type || '').split(':');
  
  let description = '';
  switch (mainType) {
    case 'download': description = 'Resume Download Credit'; break;
    case 'ai': description = 'AI Resume Optimization (includes 1 download)'; break;
    case 'subscription_freelancer': description = 'Freelancer Subscription Plan'; break;
    case 'subscription_candidate': description = 'Candidate Subscription Plan'; break;
...
export async function createAndSaveInvoice(data: any) {
  try {
